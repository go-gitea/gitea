...
GO_DIRS_OWN := cmd integrations models modules routers build services
GO_DIRS := $(GO_DIRS_OWN) vendor
...
revive:
	@echo "running revive..."
	@GO111MODULE=on $(GO) run -mod=vendor build/lint.go -config .revive.toml $(GO_SOURCES_OWN) || exit 1
