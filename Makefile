...
playwright: deps-frontend
	@# on GitHub Actions VMs, chromium and firefox system deps are pre-installed, but webkit's are not
	@pnpm exec playwright install $(if $(GITHUB_ACTIONS),,--with-deps) chromium firefox $(PLAYWRIGHT_FLAGS)
	@pnpm exec playwright install --with-deps webkit $(PLAYWRIGHT_FLAGS)
