...
func AddSystemHook(ctx *context.APIContext, form *api.CreateHookOption) {
	...
	if ok {
		h, err := webhook_service.ToHook(setting.AppSubURL+setting.AdminRouterPrefix, hook)
		if err != nil {
...
func EditSystemHook(ctx *context.APIContext, form *api.EditHookOption, hookID int64) {
	...
	h, err := webhook_service.ToHook(setting.AppURL+setting.AdminRouterPrefix, updated)
	if err != nil {
		ctx.Error(http.StatusInternalServerError, "convert.ToHook", err)
		return
	}
