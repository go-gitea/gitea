...
func TestCodespaceTokenRouteGuard(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:   "self",
			policy: codespaceTokenRoutePolicySelf,
		},
		{
			name:   "public-info",
			policy: codespaceTokenRoutePolicyPublicInfo,
		},
		{
			name:   "signed-artifact",
			policy: codespaceTokenRoutePolicySignedArtifact,
		},
		{
			name:       "unmarked-route",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ctx.GetData()[codespace_model.GiteaTokenAuthDataKey] = testCodespaceTokenSnapshot{repoID: 1}
			if tc.policy != "" {
				ctx.GetData()[codespaceTokenRoutePolicyDataKey] = tc.policy
			}
	...
	t.Run("repository-route", func(t *testing.T) {
		ctx, _ := contexttest.MockAPIContext(t, "GET /api/v1/repos/user2/repo1")
		ctx.GetData()[codespace_model.GiteaTokenAuthDataKey] = testCodespaceTokenSnapshot{repoID: 1}
		ctx.GetData()[codespaceTokenRepositoryRouteDataKey] = true
		codespaceTokenRouteGuard(ctx)
...
func TestSignedArtifactRouteSkipsRegularAPIAuth(t *testing.T) {
	...
	ctx.GetData()[codespaceTokenRoutePolicyDataKey] = codespaceTokenRoutePolicySignedArtifact
