...
func EditPullRequest(ctx *context.APIContext) {
	...
	if !issue.IsPoster(ctx.User.ID) && !ctx.Repo.CanWrite(unit.UnitTypePullRequests) {
	...
	if ctx.Repo.CanWrite(unit.UnitTypePullRequests) && (form.Assignees != nil || len(form.Assignee) > 0) {
	...
	if ctx.Repo.CanWrite(unit.UnitTypePullRequests) && form.Milestone != 0 &&
	...
	if ctx.Repo.CanWrite(unit.UnitTypePullRequests) && form.Labels != nil {
		labels, err := models.GetLabelsInRepoByIDs(ctx.Repo.Repository.ID, form.Labels)
		if err != nil {
...
func parseCompareInfo(ctx *context.APIContext, form api.CreatePullRequestOption) (*models.User, *models.Repository, *git.Repository, *git.CompareInfo, string, string) {
	...
	if !permBase.CanReadIssuesOrPulls(true) || !permBase.CanRead(unit.UnitTypeCode) {
		if log.IsTrace() {
	...
	if !permHead.CanRead(unit.UnitTypeCode) {
		if log.IsTrace() {
