...
func MustEnableProjects(ctx *context.Context) {
	if unit.UnitTypeProjects.UnitGlobalDisabled() {
		ctx.NotFound("EnableKanbanBoard", nil)
		return
	}

	if ctx.Repo.Repository != nil {
		if !ctx.Repo.CanRead(unit.UnitTypeProjects) {
...
func Projects(ctx *context.Context) {
	...
	ctx.Data["CanWriteProjects"] = ctx.Repo.Permission.CanWrite(unit.UnitTypeProjects)
...
func DeleteProjectBoard(ctx *context.Context) {
	...
	if !ctx.Repo.IsOwner() && !ctx.Repo.IsAdmin() && !ctx.Repo.CanAccess(models.AccessModeWrite, unit.UnitTypeProjects) {
		ctx.JSON(http.StatusForbidden, map[string]string{
