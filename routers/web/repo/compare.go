...
func parseCompareInfo(ctx *context.Context) *repo_service.CompareInfo {
	...
	if err != nil {
		if repo_service.IsErrCompareNotFound(err) {
			err2 := err.(repo_service.ErrCompareNotFound)

			if err2.Func == "IsBaseBranchEmptySHA" {
				if ci.IsSameRepo() {
