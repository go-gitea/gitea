...
// mustMarshalPRPayload serializes a pull request event payload for use as ActionRun.EventPayload.
func mustMarshalPRPayload(t *testing.T, payload *api.PullRequestPayload) string {
	t.Helper()
...
func TestViewPullRequestFromRun(t *testing.T) {
	...
	t.Run("pull ref", func(t *testing.T) {
		...
		assert.Equal(t, &ViewPullRequest{Index: "#123", Link: "/owner/repo/pulls/123"}, viewPullRequestFromRun(t.Context(), run))
	})
	...
	t.Run("pull request event payload", func(t *testing.T) {
		...
		run := &actions_model.ActionRun{
			...
			EventPayload: mustMarshalPRPayload(t, &api.PullRequestPayload{Index: 42}),
		}
		assert.Equal(t, &ViewPullRequest{Index: "#42", Link: "/owner/repo/pulls/42"}, viewPullRequestFromRun(t.Context(), run))
	})

	t.Run("nil repo", func(t *testing.T) {
		run := &actions_model.ActionRun{Ref: "refs/pull/1/head"}
		assert.Nil(t, viewPullRequestFromRun(t.Context(), run))
	})
}
...
func TestViewSummaryBranchFromRun(t *testing.T) {
	...
	t.Run("pull request event same repo", func(t *testing.T) {
		run := &actions_model.ActionRun{
			...
			Event: webhook_module.HookEventPullRequest,
			EventPayload: mustMarshalPRPayload(t, &api.PullRequestPayload{
		...
		assert.Equal(t, ViewBranch{Name: "feature", Link: "/owner/repo/src/branch/feature"}, viewSummaryBranchFromRun(t.Context(), run))
	})
	...
	t.Run("pull request event from fork prefixes owner", func(t *testing.T) {
		...
		assert.Equal(t, ViewBranch{Name: "forkowner:feature", Link: "/forkowner/repo/src/branch/feature"}, viewSummaryBranchFromRun(t.Context(), run))
	})

	t.Run("push to tag does not query branch", func(t *testing.T) {
		// a tag ref is not a branch, so no GetBranch DB lookup happens
		run := &actions_model.ActionRun{Repo: repo, Ref: "refs/tags/v1.0.0", Event: webhook_module.HookEventPush}
		assert.Equal(t, ViewBranch{Name: "v1.0.0", Link: "/owner/repo/src/tag/v1.0.0"}, viewSummaryBranchFromRun(t.Context(), run))
	})
}
