...
func ArtifactsDownloadView(ctx *context_module.Context) {
	...
	if len(artifacts) == 1 && actions.IsArtifactV4(artifacts[0]) {
		ctx.Resp.Header().Set("Content-Disposition", fmt.Sprintf("attachment; filename=%s; filename*=UTF-8''%s", url.PathEscape(artifacts[0].ArtifactPath), artifacts[0].ArtifactPath))
