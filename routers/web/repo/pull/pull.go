...
func ViewPullFiles(ctx *context.Context) {
	...
	code.SetCompareContext(ctx, baseCommit, commit, ctx.Repo.Owner.Name, ctx.Repo.Repository.Name)
...
func UpdatePullRequest(ctx *context.Context) {
	...
	if err = pull_service.Update(ctx, issue.PullRequest, ctx.Doer, message, rebase); err != nil {
		if models.IsErrMergeConflicts(err) {
			...
			flashError, err := ctx.RenderToString(common.TplAlertDetails, map[string]interface{}{
...
func CompareAndPullRequestPost(ctx *context.Context) {
	...
	labelIDs, assigneeIDs, milestoneID, _ := common.ValidateRepoMetas(ctx, *form, true)
	if ctx.Written() {
		return
	}
	...
	if ctx.HasError() {
		...
		code.PrepareCompareDiff(ctx, ci,
		...
		ctx.HTML(http.StatusOK, code.TplCompare)
		return
	}
	...
	if util.IsEmptyString(form.Title) {
		...
		ctx.RenderWithErr(ctx.Tr("repo.issues.new.title_empty"), code.TplCompare, form)
		return
	}
