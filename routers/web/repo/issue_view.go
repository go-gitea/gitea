...
func (prInfo *pullRequestViewInfo) prepareMergeBoxProtectedRules(ctx *context.Context) {
	...
	if data.isBlockedByApprovals {
		...
		data.infoProtectionBlockers.AddInfoItem(svg.RenderHTML("octicon-x"), blockerInfo)
	}

	data.isBlockedByRejection = issues_model.MergeBlockedByRejectedReview(ctx, pb, pull)
	if data.isBlockedByRejection {
		data.infoProtectionBlockers.AddInfoItem(svg.RenderHTML("octicon-x"), ctx.Locale.Tr("repo.pulls.blocked_by_rejection"))
	}
	...
	if data.isBlockedByOfficialReviewRequests {
		data.infoProtectionBlockers.AddInfoItem(svg.RenderHTML("octicon-x"), ctx.Locale.Tr("repo.pulls.blocked_by_official_review_requests"))
	}

	data.isBlockedByOutdatedBranch = issues_model.MergeBlockedByOutdatedBranch(pb, pull)
	if data.isBlockedByOutdatedBranch {
		data.infoProtectionBlockers.AddInfoItem(svg.RenderHTML("octicon-x"), ctx.Locale.Tr("repo.pulls.blocked_by_outdated_branch"))
	}
