...
import (
	...
	repo_router "code.gitea.io/gitea/routers/web/repo/repo"
...
func viewPullFiles(ctx *context.Context, specifiedStartCommit, specifiedEndCommit string, willShowSpecifiedCommitRange, willShowSpecifiedCommit bool) {
	...
	repo_router.SetCompareContext(ctx, baseCommit, commit, ctx.Repo.Owner.Name, ctx.Repo.Repository.Name)
...
func UpdatePullRequest(ctx *context.Context) {
	...
	if err = pull_service.Update(ctx, issue.PullRequest, ctx.Doer, message, rebase); err != nil {
		if models.IsErrMergeConflicts(err) {
			...
			flashError, err := ctx.RenderToString(base_router.TplAlertDetails, map[string]any{
...
func CompareAndPullRequestPost(ctx *context.Context) {
	...
	ci := repo_router.ParseCompareInfo(ctx)
	defer func() {
