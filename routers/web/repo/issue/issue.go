...
func MustAllowUserComment(ctx *context.Context) {
	issue := common.GetActionIssue(ctx)
	if ctx.Written() {
		return
	}
...
// List renderer issue list page according conditions
func List(ctx *context.Context, milestoneID, projectID int64, isPullOption util.OptionalBool) {
	var err error
...
func Issues(ctx *context.Context) {
	...
	List(ctx, ctx.FormInt64("milestone"), ctx.FormInt64("project"), util.OptionalBoolOf(isPullList))
	if ctx.Written() {
		return
	}
...
func NewIssue(ctx *context.Context) {
	...
	common.RetrieveRepoMetas(ctx, ctx.Repo.Repository, false)

	_, templateErrs := ctx.IssueTemplatesErrorsFromDefaultBranch()
	if errs := code.SetTemplateIfExists(ctx, issueTemplateKey, IssueTemplateCandidates); len(errs) > 0 {
		for k, v := range errs {
			templateErrs[k] = v
		}
	}
	...
	if len(templateErrs) > 0 {
		ctx.Flash.Warning(common.RenderErrorOfTemplates(ctx, templateErrs), true)
	}
...
func NewIssueChooseTemplate(ctx *context.Context) {
	...
	if len(errs) > 0 {
		ctx.Flash.Warning(common.RenderErrorOfTemplates(ctx, errs), true)
	}
...
func NewIssuePost(ctx *context.Context) {
	...
	labelIDs, assigneeIDs, milestoneID, projectID := common.ValidateRepoMetas(ctx, *form, false)
	if ctx.Written() {
		return
	}
...
func ViewIssue(ctx *context.Context) {
	...
	if issue.IsPull {
		if issue.PullRequest.HasMerged {
			...
			pull.PrepareMergedViewPullInfo(ctx, issue)
		} else {
			pull.PrepareViewPullInfo(ctx, issue)
	...
	if ctx.Repo.CanWriteIssuesOrPulls(issue.IsPull) {
		common.RetrieveRepoMilestonesAndAssignees(ctx, repo)
		common.RetrieveProjects(ctx, repo)
	...
	common.GetBranchData(ctx, issue)
	if issue.IsPull {
		...
		if (ctx.Repo.CanWriteIssuesOrPulls(issue.IsPull) || (ctx.IsSigned && issue.IsPoster(ctx.Doer.ID))) &&
			...
			if pr != nil {
				...
				if err := issue_service.ChangeStatus(issue, ctx.Doer, isClosed); err != nil {
					...
					if err := common.StopTimerIfAvailable(ctx.Doer, issue); err != nil {
