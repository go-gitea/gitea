...
func MustAllowPulls(ctx *context.Context) {
	...
	if ctx.IsSigned {
		_, has := models.HasForkedRepo(ctx.User.ID, ctx.Repo.Repository.ID)
		if has {
...
func handleTeamMentions(ctx *context.Context) {
	...
	if ctx.User.IsAdmin {
		...
		isAdmin, err = (*models.Organization)(ctx.Repo.Owner).IsOwnedBy(ctx.User.ID)
		if err != nil {
	...
	if isAdmin {
		teams, err = models.FindTeamsCtx(db.DefaultContext, ctx.Repo.Owner.ID)
		if err != nil {
			ctx.ServerError("LoadTeams", err)
			return
		}
	} else {
		teams, err = (*models.Organization)(ctx.Repo.Owner).GetUserTeams(ctx.User.ID)
		if err != nil {
