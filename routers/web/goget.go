...
func goGet(ctx *context.Context) {
	...
	parts := strings.Split(ctx.Req.URL.EscapedPath(), "/")
...
`))
	...
	repo, err := repo_model.GetRepositoryByOwnerAndName(ctx, ownerName, repoName, group)
	...
	if setting.Repository.GoGetCloneURLProtocol == "ssh" {
		cloneURL = repo_model.ComposeSSHCloneURL(ctx.Doer, ownerName, repoName, group)
	} else {
		cloneURL = repo_model.ComposeHTTPSCloneURL(ctx, ownerName, repoName, group)
	}
