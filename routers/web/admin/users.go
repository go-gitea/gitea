...
func NewUserPost(ctx *context.Context) {
	...
	ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/users/" + strconv.FormatInt(u.ID, 10))
}
...
func prepareUserInfo(ctx *context.Context) *user_model.User {
	...
	if err != nil {
		if user_model.IsErrUserNotExist(err) {
			ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/users")
		} else {
...
func EditUserPost(ctx *context.Context) {
	...
	ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/users/" + url.PathEscape(ctx.PathParam(":userid")))
}
...
func DeleteAvatar(ctx *context.Context) {
	...
	ctx.JSONRedirect(setting.AppSubURL + setting.AdminRouterPrefix + "/users/" + strconv.FormatInt(u.ID, 10))
}
