...
func DeleteRepo(ctx *context.Context) {
	...
	ctx.JSONRedirect(setting.AppSubURL + setting.AdminRouterPrefix + "/repos?page=" + url.QueryEscape(ctx.FormString("page")) + "&sort=" + url.QueryEscape(ctx.FormString("sort")))
}
...
func AdoptOrDeleteRepository(ctx *context.Context) {
	...
	if len(dirSplit) != 2 {
		ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/repos")
		return
	}
	...
	ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/repos/unadopted?search=true&q=" + url.QueryEscape(q) + "&page=" + url.QueryEscape(page))
}
