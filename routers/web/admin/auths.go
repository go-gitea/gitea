...
func NewAuthSourcePost(ctx *context.Context) {
	...
	ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/auths")
}
...
func EditAuthSourcePost(ctx *context.Context) {
	...
	ctx.Redirect(setting.AppSubURL + setting.AdminRouterPrefix + "/auths/" + strconv.FormatInt(form.ID, 10))
}
...
func DeleteAuthSource(ctx *context.Context) {
	...
	if err = auth_service.DeleteSource(ctx, source); err != nil {
		...
		ctx.JSONRedirect(setting.AppSubURL + setting.AdminRouterPrefix + "/auths/" + url.PathEscape(ctx.PathParam(":authid")))
		return
	}
	log.Trace("Authentication deleted by admin(%s): %d", ctx.Doer.Name, source.ID)

	ctx.Flash.Success(ctx.Tr("admin.auths.deletion_success"))
	ctx.JSONRedirect(setting.AppSubURL + setting.AdminRouterPrefix + "/auths")
}
