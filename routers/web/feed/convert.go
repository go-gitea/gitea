...
func toIssueLink(act *activities_model.Action) string {
	return act.GetRepoAbsoluteLink() + "/issues/" + url.PathEscape(act.GetIssueIndex())
}

func toPullLink(act *activities_model.Action) string {
	return act.GetRepoAbsoluteLink() + "/pulls/" + url.PathEscape(act.GetIssueIndex())
}
...
func feedActionsToFeedItems(ctx *context.Context, actions activities_model.ActionList) (items []*feeds.Item, err error) {
	for _, act := range actions {
		...
		case activities_model.ActionCreateIssue:
			...
			title += ctx.TrHTMLEscapeArgs("action.create_issue", link.Href, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionCreatePullRequest:
			link.Href = toPullLink(act)
			title += ctx.TrHTMLEscapeArgs("action.create_pull_request", link.Href, act.GetIssueIndex(), act.ShortRepoPath())
		...
		case activities_model.ActionCommentIssue:
			...
			title += ctx.TrHTMLEscapeArgs("action.comment_issue", issueLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionMergePullRequest:
			pullLink := toPullLink(act)
			if link.Href == "#" {
				link.Href = pullLink
			}
			title += ctx.TrHTMLEscapeArgs("action.merge_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionAutoMergePullRequest:
			pullLink := toPullLink(act)
			if link.Href == "#" {
				link.Href = pullLink
			}
			title += ctx.TrHTMLEscapeArgs("action.auto_merge_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionCloseIssue:
			issueLink := toIssueLink(act)
			if link.Href == "#" {
				link.Href = issueLink
			}
			title += ctx.TrHTMLEscapeArgs("action.close_issue", issueLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionReopenIssue:
			issueLink := toIssueLink(act)
			if link.Href == "#" {
				link.Href = issueLink
			}
			title += ctx.TrHTMLEscapeArgs("action.reopen_issue", issueLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionClosePullRequest:
			pullLink := toPullLink(act)
			if link.Href == "#" {
				link.Href = pullLink
			}
			title += ctx.TrHTMLEscapeArgs("action.close_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionReopenPullRequest:
			pullLink := toPullLink(act)
			if link.Href == "#" {
				link.Href = pullLink
			}
			title += ctx.TrHTMLEscapeArgs("action.reopen_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		...
		case activities_model.ActionApprovePullRequest:
			pullLink := toPullLink(act)
			title += ctx.TrHTMLEscapeArgs("action.approve_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionRejectPullRequest:
			pullLink := toPullLink(act)
			title += ctx.TrHTMLEscapeArgs("action.reject_pull_request", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		case activities_model.ActionCommentPull:
			pullLink := toPullLink(act)
			title += ctx.TrHTMLEscapeArgs("action.comment_pull", pullLink, act.GetIssueIndex(), act.ShortRepoPath())
		...
		case activities_model.ActionPullReviewDismissed:
			pullLink := toPullLink(act)
			title += ctx.TrHTMLEscapeArgs("action.review_dismissed", pullLink, act.GetIssueIndex(), act.ShortRepoPath(), act.GetIssueInfos()[1])
		...
		// description & content
		{
			...
			case activities_model.ActionCreateIssue, activities_model.ActionCreatePullRequest:
				desc = fmt.Sprintf("%d#%s", act.IssueIndex, act.GetIssueTitle())
