...
func TestLogsReturnsJSONPage(t *testing.T) {
	...
	assert.True(t, body.OperationActive)
	assert.True(t, body.Truncated)
	require.Len(t, body.Lines, 1)
	assert.Equal(t, "first", body.Lines[0].Message)
	assert.InDelta(t, float64(time.Date(2026, 7, 22, 3, 0, 0, 0, time.UTC).Unix()), body.Lines[0].Timestamp, 1e-9)
}
...
func TestLogsHideOtherCreatorCodespace(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
...
func TestLogPageAndDownloadUseStoredContent(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())
...
func TestDownloadLogsReadsAllPages(t *testing.T) {
	require.NoError(t, unittest.PrepareTestDatabase())

	manager := insertWebOpenManager(t, "https://gateway.example.com")
	codespaceUUID := "96909090-9090-4090-8090-909090909090"
	insertWebLogCodespace(t, manager.ID, codespaceUUID, 97)
	lines := make([]*codespacev1.LogLine, 9)
	for i := range lines {
		lines[i] = &codespacev1.LogLine{
			TimestampUnixNano: time.Date(2026, 7, 22, 3, 3, i, 0, time.UTC).UnixNano(),
			Message:           strconv.Itoa(i) + strings.Repeat("x", 60*1024),
		}
	}
	_, err := codespace_service.UpdateLog(t.Context(), manager, codespace_service.UpdateLogOptions{
		CodespaceUUID:     codespaceUUID,
		OperationRVersion: 97,
		Offset:            0,
		Lines:             lines,
	})
	require.NoError(t, err)
	...
	require.Equal(t, http.StatusOK, resp.Code)
	assert.Greater(t, resp.Body.Len(), int(codespace_service.LogReadMaxBytes))
	body := resp.Body.String()
	for i := range lines {
		assert.Contains(t, body, "] "+strconv.Itoa(i)+strings.Repeat("x", 32))
	}
}
