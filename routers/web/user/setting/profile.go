...
func UpdateAvatarSetting(ctx *context.Context, form *forms.AvatarForm, ctxUser *user_model.User) error {
	...
	if err := user_model.UpdateUserCols(ctxUser, "avatar", "avatar_email", "use_custom_avatar"); err != nil {
...
func Repos(ctx *context.Context) {
	...
	if adoptOrDelete {
		...
		root := filepath.Join(user_model.UserPath(ctxUser.Name))
		...
		userRepos, err := models.GetRepositories(ctxUser, db.ListOptions{Page: 1, PageSize: setting.UI.Admin.UserPagingNum}, repoNames...)
		if err != nil {
		...
		userRepos, count64, err := models.GetUserRepositories(&models.SearchRepoOptions{Actor: ctxUser, Private: true, ListOptions: opts})
		if err != nil {
			ctx.ServerError("GetRepositories", err)
			return
		}
