...
func KeysPost(ctx *context.Context) {
	...
	switch form.Type {
	case "principal":
		content, err := keys_model.CheckPrincipalKeyString(ctx.User, form.Content)
		if err != nil {
		...
		if _, err = keys_model.AddPrincipalKey(ctx.User.ID, content, 0); err != nil {
			ctx.Data["HasPrincipalError"] = true
			switch {
			case keys_model.IsErrKeyAlreadyExist(err), keys_model.IsErrKeyNameAlreadyUsed(err):
				loadKeysData(ctx)
	...
	case "gpg":
		...
		keys, err := keys_model.AddGPGKey(ctx.User.ID, form.Content, token, form.Signature)
		if err != nil && keys_model.IsErrGPGInvalidTokenSignature(err) {
			keys, err = keys_model.AddGPGKey(ctx.User.ID, form.Content, lastToken, form.Signature)
		}
		if err != nil {
			...
			case keys_model.IsErrGPGInvalidTokenSignature(err):
				...
				ctx.Data["KeyID"] = err.(keys_model.ErrGPGInvalidTokenSignature).ID
			...
			case keys_model.IsErrGPGNoEmailFound(err):
				...
				ctx.Data["Err_Content"] = true
				ctx.Data["Err_Signature"] = true
				ctx.Data["KeyID"] = err.(keys_model.ErrGPGNoEmailFound).ID
	...
	case "verify_gpg":
		...
		keyID, err := keys_model.VerifyGPGKey(ctx.User.ID, form.KeyID, token, form.Signature)
		if err != nil && keys_model.IsErrGPGInvalidTokenSignature(err) {
			keyID, err = keys_model.VerifyGPGKey(ctx.User.ID, form.KeyID, lastToken, form.Signature)
		}
		if err != nil {
	...
	case "ssh":
		...
		if _, err = keys_model.AddPublicKey(ctx.User.ID, form.Title, content, 0); err != nil {
			...
			case keys_model.IsErrKeyUnableVerify(err):
				ctx.Flash.Info(ctx.Tr("form.unable_verify_ssh_key"))
				ctx.Redirect(setting.AppSubURL + "/user/settings/keys")
			default:
				ctx.ServerError("AddPublicKey", err)
...
func DeleteKey(ctx *context.Context) {
	...
	case "gpg":
		if err := keys_model.DeleteGPGKey(ctx.User, ctx.FormInt64("id")); err != nil {
	...
	case "ssh":
		...
		external, err := keys_model.PublicKeyIsExternallyManaged(keyID)
		if err != nil {
		...
		if err := keys_service.DeletePublicKey(ctx.User, keyID); err != nil {
			ctx.Flash.Error("DeletePublicKey: " + err.Error())
		} else {
			ctx.Flash.Success(ctx.Tr("settings.ssh_key_deletion_success"))
		}
	case "principal":
		if err := keys_service.DeletePublicKey(ctx.User, ctx.FormInt64("id")); err != nil {
			ctx.Flash.Error("DeletePublicKey: " + err.Error())
		} else {
...
func loadKeysData(ctx *context.Context) {
	keys, err := keys_model.ListPublicKeys(ctx.User.ID, db.ListOptions{})
	if err != nil {
	...
	externalKeys, err := keys_model.PublicKeysAreExternallyManaged(keys)
	if err != nil {
		ctx.ServerError("ListPublicKeys", err)
		return
	}
	...
	gpgkeys, err := keys_model.ListGPGKeys(db.DefaultContext, ctx.User.ID, db.ListOptions{})
	if err != nil {
		ctx.ServerError("ListGPGKeys", err)
		return
	}
	ctx.Data["GPGKeys"] = gpgkeys
	tokenToSign := keys_model.VerificationToken(ctx.User, 1)

	// generate a new aes cipher using the csrfToken
	ctx.Data["TokenToSign"] = tokenToSign

	principals, err := keys_model.ListPrincipalKeys(ctx.User.ID, db.ListOptions{})
	if err != nil {
