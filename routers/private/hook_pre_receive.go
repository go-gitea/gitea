...
func preReceiveBranch(ctx *preReceiveContext, oldCommitID, newCommitID, refFullName string) {
	...
	if git.EmptySHA != oldCommitID {
		output, _, err := git.NewCommand(ctx, "rev-list", "--max-count=1").AddUntrustedArguments(oldCommitID, "^"+newCommitID).RunStdString(&git.RunOpts{Dir: repo.RepoPath(), Env: ctx.env})
		if err != nil {
