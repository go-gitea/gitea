...
func TestAPISearchRepo(t *testing.T) {
	...
	user := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 15}).(*user_model.User)
	user2 := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 16}).(*user_model.User)
	user3 := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 18}).(*user_model.User)
	user4 := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 20}).(*user_model.User)
	orgUser := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 17}).(*user_model.User)
...
func TestAPIOrgRepos(t *testing.T) {
	defer prepareTestEnv(t)()
	user := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 2}).(*user_model.User)
	user2 := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 1}).(*user_model.User)
	user3 := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 5}).(*user_model.User)
	// User3 is an Org. Check their repos.
	sourceOrg := db.AssertExistsAndLoadBean(t, &user_model.User{ID: 3}).(*user_model.User)
...
func TestAPIRepoMigrate(t *testing.T) {
	...
	defer prepareTestEnv(t)()
	for _, testCase := range testCases {
		user := db.AssertExistsAndLoadBean(t, &user_model.User{ID: testCase.ctxUserID}).(*user_model.User)
		session := loginUser(t, user.Name)
		token := getTokenForLoggedInUser(t, session)
