...
func TestViewRepoWithSymlinks(t *testing.T) {
	...
	items := files.Map(func(i int, s *goquery.Selection) string {
		cls, _ := s.Find("SVG USE").Attr("xlink:href")
		file := strings.Trim(s.Find("A").Text(), " \t\n")
		return fmt.Sprintf("%s: %s", file, strings.Split(cls, "#")[1])
	})
	assert.Equal(t, len(items), 5)
	assert.Equal(t, items[0], "a: octicon-file-directory")
	assert.Equal(t, items[1], "link_b: octicon-file-symlink-directory")
	assert.Equal(t, items[2], "link_d: octicon-file-symlink-file")
	assert.Equal(t, items[3], "link_hi: octicon-file-symlink-file")
	assert.Equal(t, items[4], "link_link: octicon-file-symlink-file")
}
