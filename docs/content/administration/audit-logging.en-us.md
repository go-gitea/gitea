...
```json
{
  ...
  "actor": {"type": "user", "id": 2, "name": "alice"},
...
The audit core does not interpret domain objects. Call sites (or domain-specific helpers) provide metadata fields appropriate to each action.
Over time, helpers should move into the packages that own the audited resources.
