...
- `SSH_SERVER_HOST_KEY`: **ssh/gogs.rsa**: For the built-in SSH server, choose the keypair to offer as the host key. The private key should be at `SSH_SERVER_HOST_KEY` and the public `SSH_SERVER_HOST_KEY.pub`. Relative paths are made absolute relative to the `APP_DATA_PATH`. If the key does not exist a 4096 bit RSA key will be created for you.
