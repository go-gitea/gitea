...
- `ALLOWED_DOMAINS`: **"*"**: Domains allowlist for migrating repositories, default is "*", which means all domains are allowed. Multiple domains could be separated by commas. Wildcard is supported: `github.com, *.github.com`.
- `BLOCKED_DOMAINS`: **\<empty\>**: Domains blocklist for migrating repositories, default is blank. Multiple domains could be separated by commas. This option has a higher priority to deny domains. Wildcard is supported.
