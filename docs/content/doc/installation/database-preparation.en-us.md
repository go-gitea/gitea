...
3.  Create database with UTF-8 charset and collation. It is recommended to use `utf8mb4` charset instead of `utf8` for complete character coverage support (including emojis). Also, collation chosen depending on your expected content. When in doubt, use either `unicode_ci` or `general_ci`.
