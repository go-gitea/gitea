...
Gitea will by default look for a number of things from the `CustomPath`, which is by default
...
These, although useful when developing, may conflict with downstream users preferences.
