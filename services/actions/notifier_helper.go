...
func (input *notifyInput) Notify(ctx context.Context) {
	...
	if err := notifyFunc(ctx, input); err != nil {
		log.Error("an error occurred while executing the %s actions method: %v", getMethod(ctx), err)
	}
}

// notifyFunc the name addes suffix Func to avoid name conflict with the notify package
func notifyFunc(ctx context.Context, input *notifyInput) error {
