...
func GenerateGiteaContext(ctx context.Context, run *actions_model.ActionRun, attempt *actions_model.ActionRunAttempt, job *actions_model.ActionRunJob) GiteaContext {
	...
	refProtected := false
	if refName.IsBranch() {
		protected, err := git_model.IsBranchProtected(ctx, run.RepoID, refName.ShortName())
		if err != nil {
	...
	gitContext := GiteaContext{
		...
		"ref_protected":     refProtected,                             // boolean, true if branch protections are configured for the ref that triggered the workflow run.
