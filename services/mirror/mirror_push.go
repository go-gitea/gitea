...
func AddPushMirrorRemote(ctx context.Context, m *repo_model.PushMirror, addr string) error {
	addRemoteAndConfig := func(addr, path string) error {
		...
		if _, _, err := cmd.RunWithContextString(&git.RunContext{Dir: path}); err != nil {
			return err
		}
		if _, _, err := git.NewCommand(ctx, "config", "--add", "remote."+m.RemoteName+".push", "+refs/heads/*:refs/heads/*").RunWithContextString(&git.RunContext{Dir: path}); err != nil {
			return err
		}
		if _, _, err := git.NewCommand(ctx, "config", "--add", "remote."+m.RemoteName+".push", "+refs/tags/*:refs/tags/*").RunWithContextString(&git.RunContext{Dir: path}); err != nil {
			return err
		}
		return nil
	}
...
func RemovePushMirrorRemote(ctx context.Context, m *repo_model.PushMirror) error {
	...
	if _, _, err := cmd.RunWithContextString(&git.RunContext{Dir: m.Repo.RepoPath()}); err != nil {
		return err
	}

	if m.Repo.HasWiki() {
		if _, _, err := cmd.RunWithContextString(&git.RunContext{Dir: m.Repo.WikiPath()}); err != nil {
