...
func checkHooks(ctx context.Context, logger log.Logger, autofix bool) error {
	if err := iterateRepositories(ctx, func(repo *repo_model.Repository) error {
		...
		if len(results) > 0 && autofix {
			logger.Warn("Regenerated hooks for %s", gitrepo.RepoGitURL(repo))
...
func checkEnablePushOptions(ctx context.Context, logger log.Logger, autofix bool) error {
	...
	if err := iterateRepositories(ctx, func(repo *repo_model.Repository) error {
		...
		if autofix {
			cmd := git.NewCommand(ctx, "config", "receive.advertisePushOptions", "true")
			_, _, err := gitrepo.RunGitCmdStdString(repo, cmd, &gitrepo.RunOpts{})
			return err
		}

		cmd := git.NewCommand(ctx, "config", "receive.advertisePushOptions")
		value, _, err := gitrepo.RunGitCmdStdString(repo, cmd, &gitrepo.RunOpts{})
		if err != nil {
			return err
		}
