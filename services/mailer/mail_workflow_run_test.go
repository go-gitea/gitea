...
func TestWorkflowRunMail(t *testing.T) {
	t.Run("StatusPresentation", func(t *testing.T) {
		...
		for _, testCase := range testCases {
			t.Run(testCase.status.String(), func(t *testing.T) {
				icon, alt, class := workflowRunJobStatusPresentation(testCase.status)
				assert.Equal(t, testCase.icon, icon)
				assert.Equal(t, testCase.alt, alt)
	...
	t.Run("Compose", func(t *testing.T) {
		...
		assert.Equal(t, 2, strings.Count(message.Body, `width="16" height="16"`))

		var serialized bytes.Buffer
		_, err := message.ToMessage().WriteTo(&serialized)
		require.NoError(t, err)
