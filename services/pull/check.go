...
func checkAndUpdateStatus(ctx context.Context, pr *issues_model.PullRequest) {
	...
	log.Trace("Updating %-v: Status:%d Conflicts:%s Protected:%s", pr, pr.Status, pr.ConflictedFiles, pr.ChangedProtectedFiles)
...
func testPR(id int64) {
	...
	log.Trace("%-v: patch tested new Status:%d ConflictedFiles:%s ChangedProtectedFiles:%s", pr, pr.Status, pr.ConflictedFiles, pr.ChangedProtectedFiles)
...
func Init() error {
	prPatchCheckerQueue = queue.CreateUniqueQueue(queue.PRPatchQueueName, handle, "")
