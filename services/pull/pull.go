...
func CloseBranchPulls(ctx context.Context, doer *user_model.User, repoID int64, branch string) error {
	...
	var errs errlist
	for _, pr := range prs {
		if err = issue_service.CloseIssue(ctx, pr.Issue, doer, ""); err != nil && !issues_model.IsErrIssueWasClosed(err) && !issues_model.IsErrDependenciesLeft(err) {
			errs = append(errs, err)
		}
	}
	if len(errs) > 0 {
...
func CloseRepoBranchesPulls(ctx context.Context, doer *user_model.User, repo *repo_model.Repository) error {
	...
	for _, branch := range branches {
		...
		for _, pr := range prs {
			...
			if err = issue_service.CloseIssue(ctx, pr.Issue, doer, ""); err != nil && !issues_model.IsErrIssueWasClosed(err) {
				errs = append(errs, err)
			}
		}
	}
