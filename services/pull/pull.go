...
func CloseBranchPulls(doer *user_model.User, repoID int64, branch string) error {
	...
	var errs errlist
	for _, pr := range prs {
		if err = issue_service.ChangeStatus(pr.Issue, doer, "", true, issues_model.IssueStatusClosed); err != nil && !issues_model.IsErrPullWasClosed(err) && !issues_model.IsErrDependenciesLeft(err) {
...
func CloseRepoBranchesPulls(ctx context.Context, doer *user_model.User, repo *repo_model.Repository) error {
	...
	for _, branch := range branches {
		...
		for _, pr := range prs {
			...
			if err = issue_service.ChangeStatus(pr.Issue, doer, "", true, issues_model.IssueStatusClosed); err != nil && !issues_model.IsErrPullWasClosed(err) {
				errs = append(errs, err)
			}
		}
	}
