...
func getRebaseAmendMessage(ctx *mergeContext, baseGitRepo *git.Repository) (message string, err error) {
	...
	commitMessage, _, err := gitcmd.New("show", "--format=%B", "-s").RunStdString(ctx, &gitcmd.RunOpts{Dir: ctx.tmpBasePath})
	if err != nil {
		return "", err
	}
...
func doMergeRebaseFastForward(ctx *mergeContext) error {
	...
	cmd := gitcmd.New("merge", "--ff-only").AddDynamicArguments(stagingBranch)
	...
	if newMessage != "" {
		if err := gitcmd.New("commit", "--amend").AddOptionFormat("--message=%s", newMessage).Run(ctx, &gitcmd.RunOpts{Dir: ctx.tmpBasePath}); err != nil {
...
func doMergeRebaseMergeCommit(ctx *mergeContext, message string) error {
	cmd := gitcmd.New("merge").AddArguments("--no-ff", "--no-commit").AddDynamicArguments(stagingBranch)
...
func doMergeStyleRebase(ctx *mergeContext, mergeStyle repo_model.MergeStyle, message string) error {
	...
	if err := gitcmd.New("checkout").AddDynamicArguments(baseBranch).
