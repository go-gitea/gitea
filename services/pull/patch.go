...
func attemptMerge(ctx context.Context, file *unmergedFile, tmpBasePath string, gitRepo *git.Repository) error {
	...
	case file.stage2 != nil && file.stage3 != nil:
		...
		root, _, err := git.NewCommand(ctx, "unpack-file").AddUntrustedArguments(file.stage1.sha).RunStdString(&git.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		base, _, err := git.NewCommand(ctx, "unpack-file").AddUntrustedArguments(file.stage2.sha).RunStdString(&git.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		head, _, err := git.NewCommand(ctx, "unpack-file").AddUntrustedArguments(file.stage3.sha).RunStdString(&git.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		_, _, conflictErr := git.NewCommand(ctx, "merge-file").AddUntrustedArguments(base, root, head).RunStdString(&git.RunOpts{Dir: tmpBasePath})
		if conflictErr != nil {
			return &errMergeConflict{file.stage2.path}
		}

		// base now contains the merged data
		hash, _, err := git.NewCommand(ctx, "hash-object", "-w", "--path").AddUntrustedArguments(file.stage2.path, base).RunStdString(&git.RunOpts{Dir: tmpBasePath})
		if err != nil {
			return err
		}
...
func AttemptThreeWayMerge(ctx context.Context, gitPath string, gitRepo *git.Repository, base, ours, theirs, description string) (bool, []string, error) {
	...
	if _, _, err := git.NewCommand(ctx, "read-tree", "-m").AddUntrustedArguments(base, ours, theirs).RunStdString(&git.RunOpts{Dir: gitPath}); err != nil {
