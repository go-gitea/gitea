...
func TestPatch(pr *models.PullRequest) error {
	...
	// 1. update merge base
	pr.MergeBase, _, err = git.NewCommand(ctx, "merge-base", "--", "base", "tracking").RunWithContextString(&git.RunContext{Dir: tmpBasePath})
	if err != nil {
		var err2 error
...
func attemptMerge(ctx context.Context, file *unmergedFile, tmpBasePath string, gitRepo *git.Repository) error {
	...
	case file.stage2 != nil && file.stage3 != nil:
		...
		root, _, err := git.NewCommand(ctx, "unpack-file", file.stage1.sha).RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if err != nil {
		...
		base, _, err := git.NewCommand(ctx, "unpack-file", file.stage2.sha).RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if err != nil {
		...
		head, _, err := git.NewCommand(ctx, "unpack-file", file.stage3.sha).RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if err != nil {
		...
		_, _, conflictErr := git.NewCommand(ctx, "merge-file", base, root, head).RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if conflictErr != nil {
			return &errMergeConflict{file.stage2.path}
		}

		// base now contains the merged data
		hash, _, err := git.NewCommand(ctx, "hash-object", "-w", "--path", file.stage2.path, base).RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if err != nil {
			return err
		}
...
func AttemptThreeWayMerge(ctx context.Context, gitPath string, gitRepo *git.Repository, base, ours, theirs, description string) (bool, []string, error) {
	...
	if _, _, err := git.NewCommand(ctx, "read-tree", "-m", base, ours, theirs).RunWithContextString(&git.RunContext{Dir: gitPath}); err != nil {
...
func checkConflicts(ctx context.Context, pr *models.PullRequest, gitRepo *git.Repository, tmpBasePath string) (bool, error) {
	...
	if !conflict {
		var treeHash string
		treeHash, _, err = git.NewCommand(ctx, "write-tree").RunWithContextString(&git.RunContext{Dir: tmpBasePath})
		if err != nil {
			return false, err
		}
	...
	_, _, err = git.NewCommand(gitRepo.Ctx, "read-tree", "base").RunWithContextString(&git.RunContext{Dir: tmpBasePath})
	if err != nil {
