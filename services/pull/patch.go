...
func testPullRequestTmpRepoBranchMergeable(ctx context.Context, prCtx *prTmpRepoContext, pr *issues_model.PullRequest) error {
	...
	// 1. update merge base
	pr.MergeBase, _, err = gitcmd.New("merge-base", "--", "base", "tracking").RunStdString(ctx, &gitcmd.RunOpts{Dir: prCtx.tmpBasePath})
	if err != nil {
		var err2 error
...
func attemptMerge(ctx context.Context, file *unmergedFile, tmpBasePath string, filesToRemove *[]string, filesToAdd *[]git.IndexObjectInfo) error {
	...
	case file.stage2 != nil && file.stage3 != nil:
		...
		root, _, err := gitcmd.New("unpack-file").AddDynamicArguments(file.stage1.sha).RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		base, _, err := gitcmd.New("unpack-file").AddDynamicArguments(file.stage2.sha).RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		head, _, err := gitcmd.New("unpack-file").AddDynamicArguments(file.stage3.sha).RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if err != nil {
		...
		_, _, conflictErr := gitcmd.New("merge-file").AddDynamicArguments(base, root, head).RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if conflictErr != nil {
			return &errMergeConflict{file.stage2.path}
		}

		// base now contains the merged data
		hash, _, err := gitcmd.New("hash-object", "-w", "--path").AddDynamicArguments(file.stage2.path, base).RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if err != nil {
			return err
		}
...
func AttemptThreeWayMerge(ctx context.Context, gitPath string, gitRepo *git.Repository, base, ours, theirs, description string) (bool, []string, error) {
	...
	if _, _, err := gitcmd.New("read-tree", "-m").AddDynamicArguments(base, ours, theirs).RunStdString(ctx, &gitcmd.RunOpts{Dir: gitPath}); err != nil {
...
func checkConflicts(ctx context.Context, pr *issues_model.PullRequest, gitRepo *git.Repository, tmpBasePath string) (bool, error) {
	...
	if !conflict {
		...
		var treeHash string
		treeHash, _, err = gitcmd.New("write-tree").RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
		if err != nil {
			lsfiles, _, _ := gitcmd.New("ls-files", "-u").RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
	...
	_, _, err = gitcmd.New("read-tree", "base").RunStdString(gitRepo.Ctx, &gitcmd.RunOpts{Dir: tmpBasePath})
	if err != nil {
	...
	cmdApply := gitcmd.New("apply", "--check", "--cached")
