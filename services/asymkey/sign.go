...
func SigningKey(ctx context.Context, repoPath string) (string, *git.Signature) {
	...
	if setting.Repository.Signing.SigningKey == "default" || setting.Repository.Signing.SigningKey == "" {
		...
		value, _, _ := git.NewCommand(ctx, "config", "--get", "commit.gpgsign").RunWithContextString(&git.RunContext{Dir: repoPath})
		sign, valid := git.ParseBool(strings.TrimSpace(value))
		if !sign || !valid {
			return "", nil
		}

		signingKey, _, _ := git.NewCommand(ctx, "config", "--get", "user.signingkey").RunWithContextString(&git.RunContext{Dir: repoPath})
		signingName, _, _ := git.NewCommand(ctx, "config", "--get", "user.name").RunWithContextString(&git.RunContext{Dir: repoPath})
		signingEmail, _, _ := git.NewCommand(ctx, "config", "--get", "user.email").RunWithContextString(&git.RunContext{Dir: repoPath})
