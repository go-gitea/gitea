...
func TestUser_ToUser(t *testing.T) {
	...
	apiUser := toUser(db.DefaultContext, user1, user1, true, true)
	assert.True(t, apiUser.IsAdmin)
	assert.Contains(t, apiUser.AvatarURL, "://")

	user2 := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 2, IsAdmin: false})

	apiUser = toUser(db.DefaultContext, user2, user2, true, true)
	assert.False(t, apiUser.IsAdmin)

	apiUser = toUser(db.DefaultContext, user1, user1, false, false)
	assert.False(t, apiUser.IsAdmin)
	...
	apiUser = toUser(db.DefaultContext, user31, user31, true, true)
	assert.False(t, apiUser.IsAdmin)
