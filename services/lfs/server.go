...
func (rc *requestContext) DownloadLink(p lfs_module.Pointer) string {
	return setting.GetLFSRootURL() + path.Join(rc.User, rc.Repo+".git", "info/lfs/objects", p.Oid)
}

// UploadLink builds a URL to upload the object.
func (rc *requestContext) UploadLink(p lfs_module.Pointer) string {
	return setting.GetLFSRootURL() + path.Join(rc.User, rc.Repo+".git", "info/lfs/objects", p.Oid, strconv.FormatInt(p.Size, 10))
}

// VerifyLink builds a URL for verifying the object.
func (rc *requestContext) VerifyLink(p lfs_module.Pointer) string {
	return setting.GetLFSRootURL() + path.Join(rc.User, rc.Repo+".git", "info/lfs/verify")
}
