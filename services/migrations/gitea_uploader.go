...
func (g *GiteaLocalUploader) updateGitForPullRequest(pr *base.PullRequest) (head string, err error) {
	...
	if pr.IsForkPullRequest() && pr.State != "closed" {
		if pr.Head.OwnerName != "" {
			...
			if ok {
				_, _, err = git.NewCommand(g.ctx, "fetch", remote, pr.Head.Ref).RunWithContextString(&git.RunContext{Dir: g.repo.RepoPath()})
				if err != nil {
		...
		_, _, err = git.NewCommand(g.ctx, "rev-list", "--quiet", "-1", pr.Head.SHA).RunWithContextString(&git.RunContext{Dir: g.repo.RepoPath()})
		if err != nil {
			if pr.Head.SHA != "" {
