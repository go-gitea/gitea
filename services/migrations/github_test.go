...
func TestGitHubDownloadRepo(t *testing.T) {
	...
	comments, _, err := downloader.GetComments(&base.Issue{Number: 2, ForeignID: 2})
	assert.NoError(t, err)
	...
	reviews, err := downloader.GetReviews(&base.PullRequest{Number: 3, ForeignID: 3})
	assert.NoError(t, err)
	...
	reviews, err = downloader.GetReviews(&base.PullRequest{Number: 4, ForeignID: 4})
	assert.NoError(t, err)
	assertReviewsEqual(t, []*base.Review{
		{
			ID:           338338740,
