...
func TestGiteaUploadUpdateGitForPullRequest(t *testing.T) {
	...
	err := git.NewCommand(git.DefaultContext, "symbolic-ref", "HEAD", git.BranchPrefix+baseRef).RunWithContext(&git.RunContext{Dir: fromRepo.RepoPath(), Timeout: -1})
	assert.NoError(t, err)
	...
	//
	headRef := "branch1"
	err = git.NewCommand(git.DefaultContext, "checkout", "-b", headRef).RunWithContext(&git.RunContext{Dir: fromRepo.RepoPath(), Timeout: -1})
	assert.NoError(t, err)
	...
	err = git.NewCommand(git.DefaultContext, "checkout", "-b", forkHeadRef).RunWithContext(&git.RunContext{Dir: forkRepo.RepoPath(), Timeout: -1})
	assert.NoError(t, err)
