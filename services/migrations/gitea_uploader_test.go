...
func TestGiteaUploadUpdateGitForPullRequest(t *testing.T) {
	...
	_, _, err = git.NewCommand(git.DefaultContext, "symbolic-ref", "HEAD", git.BranchPrefix+baseRef).RunWithContextString(&git.RunContext{Dir: fromRepo.RepoPath()})
	assert.NoError(t, err)
	...
	//
	headRef := "branch1"
	_, _, err = git.NewCommand(git.DefaultContext, "checkout", "-b", headRef).RunWithContextString(&git.RunContext{Dir: fromRepo.RepoPath()})
	assert.NoError(t, err)
	...
	_, _, err = git.NewCommand(git.DefaultContext, "checkout", "-b", forkHeadRef).RunWithContextString(&git.RunContext{Dir: forkRepo.RepoPath()})
	assert.NoError(t, err)
