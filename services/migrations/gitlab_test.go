...
func TestConvertGitlabRelease(t *testing.T) {
	downloader := &GitlabDownloader{}
	now := time.Now()

	// Test case: Release with more links than sources (common with GoReleaser)
	// This should not panic - issue #36292
	t.Run("MoreLinksThanSources", func(t *testing.T) {
		release := &gitlab.Release{
			...
			Assets: struct {
				...
				// 2 sources (typical: tar.gz and zip)
				Sources: []struct {
					Format string `json:"format"`
					URL    string `json:"url"`
				}{
					{Format: "tar.gz", URL: "https://example.com/source.tar.gz"},
					{Format: "zip", URL: "https://example.com/source.zip"},
				},
				// 5 links (typical GoReleaser output: binaries + checksums)
				Links: []*gitlab.ReleaseLink{
					{ID: 10, Name: "app-linux-amd64", URL: "https://example.com/app-linux-amd64"},
					{ID: 11, Name: "app-linux-arm64", URL: "https://example.com/app-linux-arm64"},
					{ID: 12, Name: "app-darwin-amd64", URL: "https://example.com/app-darwin-amd64"},
					{ID: 13, Name: "app-windows-amd64.exe", URL: "https://example.com/app-windows-amd64.exe"},
					{ID: 14, Name: "checksums.txt", URL: "https://example.com/checksums.txt"},
				},
			},
		}

		ctx := t.Context()
		result := downloader.convertGitlabRelease(ctx, release)
	...
	t.Run("NoLinks", func(t *testing.T) {
		release := &gitlab.Release{
			...
			Assets: struct {
				...
				Links: []*gitlab.ReleaseLink{}, // No links
			},
		}
		...
		assert.NotNil(t, result)
		assert.Equal(t, "v0.1.0", result.TagName)
