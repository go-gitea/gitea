...
func (g *RepositoryDumper) handlePullRequest(ctx context.Context, pr *base.PullRequest) error {
	...
	if pr.Head.CloneURL == "" || pr.Head.Ref == "" {
		...
		if pr.Head.SHA != "" {
			_, _, err = gitcmd.New("update-ref", "--no-deref").AddDynamicArguments(pr.GetGitHeadRefName(), pr.Head.SHA).RunStdString(ctx, &gitcmd.RunOpts{Dir: g.gitPath()})
			if err != nil {
	...
	if !ok {
		...
		_, _, err = gitcmd.New("fetch", "--no-tags").AddDashesAndList(remote, fetchArg).RunStdString(ctx, &gitcmd.RunOpts{Dir: g.gitPath()})
		if err != nil {
