...
func PullRequestCodeOwnersReview(ctx context.Context, pr *issues_model.PullRequest) ([]*ReviewRequestNotifier, error) {
	...
	// Use ghost user as doer for CODEOWNERS-triggered review requests
	// to correctly attribute the action to the system rather than the PR author
	codeOwnersDoer := user_model.NewGhostUser()

	for _, u := range uniqUsers {
		if u.ID != issue.Poster.ID && !contain(latestReivews, u) {
			comment, err := issues_model.AddReviewRequest(ctx, issue, u, codeOwnersDoer)
			if err != nil {
	...
	for _, t := range uniqTeams {
		comment, err := issues_model.AddTeamReviewRequest(ctx, issue, t, codeOwnersDoer)
		if err != nil {
