...
func GetDiff(ctx context.Context, gitRepo *git.Repository, opts *DiffOptions, files ...string) (*Diff, error) {
	...
	if (len(opts.BeforeCommitID) == 0 || opts.BeforeCommitID == gitRepo.HashType.Empty()) && commit.ParentCount() == 0 {
	...
	if len(opts.BeforeCommitID) == 0 || opts.BeforeCommitID == gitRepo.HashType.Empty() {
		diffPaths = []string{gitRepo.HashType.EmptyTree(), opts.AfterCommitID}
	}
