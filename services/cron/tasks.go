...
func RegisterTask[T Config](name string, config T, fun func(context.Context, *user_model.User, T) error) error {
	...
	if value := LocaleTaskName(translation.NewLocale("en-US"), name); value == name {
		return fmt.Errorf("translation is missing for task %q, please add a LocaleTaskName case and translation", name)
	}
