	...
	for orgName, teamNames := range orgTeamMap {
		...
		for _, teamName := range teamNames {
			...
			if action == syncAdd && !isMember {
				if err := org_service.AddTeamMember(ctx, user_model.NewAuthenticationSourceUser(), team, user); err != nil {
					log.Error("group sync: Could not add user to team: %v", err)
					return err
				}
			} else if action == syncRemove && isMember {
				if err := org_service.RemoveTeamMember(ctx, user_model.NewAuthenticationSourceUser(), team, user); err != nil {
