...
func prepareGitPath(ctx context.Context, repo gitrepo.Repository, defaultWikiBranch string, wikiPath WebPath) (bool, string, error) {
	unescaped := string(wikiPath) + ".md"
	gitPath := WebPathToGitPath(wikiPath)

	// Look for both files
	filesInIndex, err := gitrepo.LsTree(ctx, repo, defaultWikiBranch, unescaped, gitPath)
	if err != nil {
...
func updateWikiPage(ctx context.Context, doer *user_model.User, repo *repo_model.Repository, oldWikiName, newWikiName WebPath, content, message string, isNew bool) (err error) {
	...
	isWikiExist, newWikiPath, err := prepareGitPath(ctx, repo.WikiStorageRepo(), repo.DefaultWikiBranch, newWikiName)
	if err != nil {
		return err
	}
	...
	if isNew {
		...
		if oldWikiName != newWikiName {
			isOldWikiExist, oldWikiPath, err = prepareGitPath(ctx, repo.WikiStorageRepo(), repo.DefaultWikiBranch, oldWikiName)
			if err != nil {
				return err
			}
		}
...
func DeleteWikiPage(ctx context.Context, doer *user_model.User, repo *repo_model.Repository, wikiName WebPath) (err error) {
	...
	found, wikiPath, err := prepareGitPath(ctx, repo.WikiStorageRepo(), repo.DefaultWikiBranch, wikiName)
	if err != nil {
		return err
	}
	if found {
