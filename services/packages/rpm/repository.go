...
func BuildRepositoryFiles(ctx context.Context, ownerID int64, compositeKey string) error {
	...
	primary, err := buildPrimary(pv, pfs, cache, compositeKey)
	if err != nil {
		return err
	}
	filelists, err := buildFilelists(pv, pfs, cache, compositeKey)
	if err != nil {
		return err
	}
	other, err := buildOther(pv, pfs, cache, compositeKey)
	if err != nil {
		return err
	}
...
func buildPrimary(pv *packages_model.PackageVersion, pfs []*packages_model.PackageFile, c packageCache, compositeKey string) (*repoData, error) {
	type Version struct {
...
func buildFilelists(pv *packages_model.PackageVersion, pfs []*packages_model.PackageFile, c packageCache, compositeKey string) (*repoData, error) { //nolint:dupl
	type Version struct {
		Epoch   string `xml:"epoch,attr"`
		Version string `xml:"ver,attr"`
		Release string `xml:"rel,attr"`
...
func buildOther(pv *packages_model.PackageVersion, pfs []*packages_model.PackageFile, c packageCache, compositeKey string) (*repoData, error) { //nolint:dupl
	type Version struct {
		Epoch   string `xml:"epoch,attr"`
		Version string `xml:"ver,attr"`
		Release string `xml:"rel,attr"`
