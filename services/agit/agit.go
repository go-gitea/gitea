...
func ProcRecive(ctx *context.PrivateContext, opts *private.HookOptions) []private.HookProcReceiveRefResult {
	...
	for i := range opts.OldCommitIDs {
		...
		if !forcePush {
			output, _, err := git.NewCommand(ctx, "rev-list", "--max-count=1", oldCommitID, "^"+opts.NewCommitIDs[i]).RunWithContextString(&git.RunContext{Dir: repo.RepoPath(), Env: os.Environ()})
			if err != nil {
