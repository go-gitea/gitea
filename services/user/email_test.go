...
func TestUserEmail(t *testing.T) {
	...
	t.Run("PrimaryEmailAddress", func(t *testing.T) {
		...
		assert.NoError(t, ReplacePrimaryEmailAddress(t.Context(), user, user, "primary-13@example.com"))
	...
	t.Run("AddEmailAddresses", func(t *testing.T) {
		...
		assert.Error(t, AddEmailAddresses(t.Context(), user, user, []string{" invalid email "}))

		emails := []string{"user1234@example.com", "user5678@example.com"}

		assert.NoError(t, AddEmailAddresses(t.Context(), user, user, emails))

		err := AddEmailAddresses(t.Context(), user, user, emails)
		assert.Error(t, err)
	...
	t.Run("DeleteEmailAddresses", func(t *testing.T) {
		...
		err := DeleteEmailAddresses(t.Context(), user, user, emails)
		assert.NoError(t, err)
