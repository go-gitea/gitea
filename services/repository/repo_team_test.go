...
func TestTeam_AddRepository(t *testing.T) {
	...
	testSuccess := func(teamID, repoID int64) {
		...
		assert.NoError(t, TeamAddRepository(t.Context(), user_model.NewGhostUser(), team, repo))
	...
	assert.Error(t, TeamAddRepository(t.Context(), user_model.NewGhostUser(), team, repo))
