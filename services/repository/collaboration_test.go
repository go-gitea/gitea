...
func TestRepository_DeleteCollaborationRemovesWatches(t *testing.T) {
	assert.NoError(t, unittest.PrepareTestDatabase())
	...
	permAfter, err := access_model.GetUserRepoPermission(ctx, repo, user)
	assert.NoError(t, err)
	assert.Equal(t, perm.AccessModeNone, permAfter.AccessMode)
