...
func prepareRepoCommit(ctx context.Context, repo *repo_model.Repository, tmpDir, repoPath string, opts CreateRepoOptions) error {
	...
	// README
	data, err := repo_module.GetRepoInitFile("readme", opts.Readme)
	if err != nil {
	...
	if len(opts.Gitignores) > 0 {
		...
		for _, name := range names {
			data, err = repo_module.GetRepoInitFile("gitignore", name)
			if err != nil {
	...
	if len(opts.License) > 0 {
		data, err = repo_module.GetRepoInitFile("license", opts.License)
		if err != nil {
...
// CreateRepositoryWithNoNotify creates a repository for the user/organization.
func CreateRepositoryWithNoNotify(doer, u *user_model.User, opts CreateRepoOptions) (*repo_model.Repository, error) {
	...
	if len(opts.IssueLabels) > 0 {
		if _, err := repo_module.GetLabelTemplateFile(opts.IssueLabels); err != nil {
			return nil, err
		}
	}
	...
	if err := db.WithTx(db.DefaultContext, func(ctx context.Context) error {
		if err := repo_module.CreateRepositoryByExample(ctx, doer, u, repo, false); err != nil {
			return err
		}
		...
		if rollbackRepo != nil {
			if errDelete := DeleteRepositoryWithNoNotify(doer, rollbackRepo.OwnerID, rollbackRepo.ID); errDelete != nil {
