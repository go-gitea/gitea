...
func GenerateRepository(doer, u *models.User, oldRepo *models.Repository, name, desc string, private bool) (*models.Repository, error) {
	repo, err := models.GenerateRepository(doer, u, oldRepo, name, desc, private)
	if err != nil {
		if repo != nil {
			if errDelete := models.DeleteRepository(doer, u.ID, repo.ID); errDelete != nil {
				log.Error("Rollback deleteRepository: %v", errDelete)
