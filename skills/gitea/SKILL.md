---
name: gitea
description: Enroll and interact with a Gitea instance using machine credentials for Git operations.
triggers:
  - enroll agent
...
After success:
1. Persist `user.username` as the canonical Gitea identity for this machine.
2. Persist `token` in secure local secret storage.
3. Expose the token as `GITEA_TOKEN` for future API/Git HTTPS calls.
4. Persist `token_scopes`, `base_url`, and bootstrap repo URL `"$GITEA_BASE_URL/<user.username>.git"`.
