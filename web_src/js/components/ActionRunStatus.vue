...
<template>
  <span :data-tooltip-content="localeStatus ?? status" v-if="status">
    <SvgIcon :name="`octicon-check${iconSuffix}`" class="tw-text-green" :size="size" :class="className" v-if="status === 'success'"/>
    ...
    <SvgIcon :name="`octicon-x${iconSuffix}`" class="tw-text-red" :size="size" :class="className" v-else/><!-- failure, unknown -->
  </span>
</template>
