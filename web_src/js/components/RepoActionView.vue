      ...
      <div class="action-view-right">
        ...
        <div class="job-step-container" ref="steps" v-if="currentJob.steps.length">
          <div class="job-step-section" v-for="(jobStep, i) in currentJob.steps" :key="i">
            <div class="job-step-summary" @click.stop="jobStep.status !== 'skipped' && toggleStepLogs(i)" :class="[currentJobStepsStates[i].expanded ? 'selected' : '', jobStep.status ? `step-status-${jobStep.status}` : '']">
              ...
              <SvgIcon v-else :name="currentJobStepsStates[i].expanded ? 'octicon-chevron-down': 'octicon-chevron-right'" :class="['gt-mr-3', jobStep.status === 'skipped' && 'tw-invisible']"/>
...
.job-step-container .job-step-summary:not(.step-status-skipped) {
  cursor: pointer;
}
...
.job-step-container .job-step-summary:not(.step-status-skipped):hover {
  color: var(--color-console-fg);
