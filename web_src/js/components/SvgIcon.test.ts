...
function render(props: InstanceType<typeof SvgIcon>['$props']): string {
  const root = document.createElement('div');
  createApp({render: () => h(SvgIcon, props)}).mount(root);
  return root.innerHTML;
}

test('SvgIcon', () => {
  expect(render({name: 'octicon-dot-fill', size: 24, class: 'base', symbolId: 'svg-symbol-dot'})).toBe(
  ...
  expect(render({useHref: '#svg-mfi-go', class: 'svg git-entry-icon octicon-file'})).toBe(
