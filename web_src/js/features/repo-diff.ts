...
function initRepoDiffFileBox(el: HTMLElement) {
  ...
  queryElems(el, '.file-view-toggle', (btn) =>
    btn.addEventListener('click', () => {
      queryElemSiblings(btn, '.file-view-toggle', (el) =>
        el.classList.remove('active'),
      );
      btn.classList.add('active');

      const target = document.querySelector(
        btn.getAttribute('data-toggle-selector')!,
      );
...
function initRepoDiffConversationForm() {
  ...
  addDelegatedEventListener<HTMLFormElement, SubmitEvent>(
    document,
    'submit',
    ...
    async (form, e) => {
      ...
      try {
        ...
        const isSubmittedByButton =
          submitter?.nodeName === 'BUTTON' ||
          (submitter?.nodeName === 'INPUT' && submitter.type === 'submit');
        ...
        const newConversationHolder = createElementFromHTML(
          await response.text(),
        );
        ...
        if (!submitter || submitter?.matches('button[name="pending_review"]')) {
          ...
          if (!counter) return;
          const num =
            parseInt(counter.getAttribute('data-pending-comment-number')!) +
              1 || 1;
    ...
    async (el, e) => {
      ...
      try {
        const response = await POST(url, {
          data: new URLSearchParams({origin, action, comment_id}),
        });
...
function initRepoDiffConversationNav() {
  ...
  addDelegatedEventListener(
    ...
    (el, e) => {
      ...
      const elAllConversations = document.querySelectorAll(
        '.comment-code-cloud:not(.tw-hidden)',
      );
      const index = Array.from(elAllConversations).indexOf(elCurConversation);
      const previousIndex =
        index > 0 ? index - 1 : elAllConversations.length - 1;
...
function initDiffHeaderPopup() {
  for (const btn of document.querySelectorAll(
    '.diff-header-popup-btn:not([data-header-popup-initialized])',
  )) {
...
async function loadMoreFiles(btn: Element): Promise<boolean> {
  ...
  try {
    ...
    document
      .querySelector('#diff-incomplete')!
      .replaceWith(...Array.from(respFileBoxes.children));
    onShowMoreFiles();
    return true;
  } catch (error) {
    console.error('Error:', error);
  ...
  addDelegatedEventListener(
    ...
    async (el, e) => {
      ...
      try {
        ...
        const resp = await response.text();
        const respDoc = parseDom(resp, 'text/html');
        const respFileBody = respDoc.querySelector(
          '#diff-file-boxes .diff-file-body .file-body',
        )!;
...
async function onLocationHashChange() {
  ...
  if (
    !currentHash.startsWith('#diff-') &&
    !currentHash.startsWith('#issuecomment-')
  ) return;
  ...
  while (currentHash === window.location.hash) {
    ...
    if (currentHash.startsWith(issueCommentPrefix)) {
      ...
      const expandButton = document.querySelector<HTMLElement>(
        `.code-expander-button[data-hidden-comment-ids*=",${commentId},"]`,
      );
      if (expandButton) {
...
export function initRepoDiffView() {
  ...
  registerGlobalSelectorFunc(
    '#diff-file-boxes .diff-file-box',
    initRepoDiffFileBox,
  );
