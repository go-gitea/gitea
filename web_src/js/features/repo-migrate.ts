...
export function initRepoMigrationForm() {
  registerGlobalInitFunc<HTMLFormElement>('initRepoMigrationForm', (form) => {
    const cloneAddrInput = form.querySelector<HTMLInputElement>('input[name="clone_addr"]');
    ...
    const userpassFields = form.querySelectorAll<HTMLElement>('.auth-userpass-field');
    const sshHelpField = form.querySelector<HTMLElement>('.ssh-help-field');
...
function initSSHKeyOwnerSelector(form: HTMLFormElement, cloneAddrInput: HTMLInputElement) {
  const selector = form.querySelector<HTMLElement>('.ssh-key-owner-selector');
  const fingerprintOnly = form.querySelector<HTMLElement>('.ssh-key-fingerprint-only');
  ...
  const keysLink = form.querySelector<HTMLAnchorElement>('.js-ssh-keys-link');
  ...
  for (const item of form.querySelectorAll<HTMLElement>('.owner.dropdown .menu .item, .ssh-key-owner-selector .menu .item')) {
