...
function initRepoIssueProjectFilter(elDropdown: HTMLElement) {
  const url = new URL(window.location.href);
  const queryProjects = url.searchParams.get('project') || url.searchParams.get('projects') || '';
  const selectedProjectIds = new Set<string>();
  for (const id of queryProjects ? queryProjects.split(',') : []) {
    const numericId = Number.parseInt(id, 10);
    if (!Number.isFinite(numericId) || numericId === 0) continue;
    selectedProjectIds.add(String(numericId));
  }
  if (selectedProjectIds.has('-1') && selectedProjectIds.size > 1) {
    selectedProjectIds.clear();
    selectedProjectIds.add('-1');
  }

  const normalizeSelectedProjectIds = () => {
    if (selectedProjectIds.size === 0) return '';
    if (selectedProjectIds.has('-1')) return '-1';
    const projectIdList = Array.from(selectedProjectIds, (id) => Number.parseInt(id, 10))
      .filter((id) => Number.isFinite(id) && id > 0);
    projectIdList.sort((a, b) => a - b);
    return projectIdList.join(',');
  };

  const initialSelection = normalizeSelectedProjectIds();

  const elAll = elDropdown.querySelector<HTMLElement>('[data-project-filter-all]');
  const elNone = elDropdown.querySelector<HTMLElement>('[data-project-filter-none]');

  const syncSelectionState = () => {
    const noProjectSelected = selectedProjectIds.size === 1 && selectedProjectIds.has('-1');
    if (elAll) {
      elAll.classList.toggle('active', selectedProjectIds.size === 0);
      elAll.classList.toggle('selected', selectedProjectIds.size === 0);
    }
    if (elNone) {
      elNone.classList.toggle('active', noProjectSelected);
      elNone.classList.toggle('selected', noProjectSelected);
    }
    queryElems(elDropdown, '[data-project-id]', (el) => {
  ...
  const applySelection = () => {
    const nextSelection = normalizeSelectedProjectIds();
    if (nextSelection === initialSelection) return;
    url.searchParams.delete('project');
    url.searchParams.delete('projects');
    if (nextSelection !== '') {
      url.searchParams.set('project', nextSelection);
    }
    window.location.assign(url);
  };

  const $dropdown = fomanticQuery(elDropdown);
  const onHide = $dropdown.dropdown('setting', 'onHide');
  $dropdown.dropdown('setting', {
    action: 'nothing',
    onHide() {
      const shouldHide = onHide?.call(this);
  ...
  if (elAll) {
    elAll.addEventListener('click', (e: MouseEvent) => {
  ...
  if (elNone) {
    elNone.addEventListener('click', (e: MouseEvent) => {
      if (e.metaKey || e.ctrlKey || e.shiftKey || e.altKey) return;
      e.preventDefault();
      selectedProjectIds.clear();
      selectedProjectIds.add('-1');
...
export function initRepoIssueFilterItemLabel() {
  ...
  queryElems(document, '.ui.dropdown.project-filter', initRepoIssueProjectFilter);
}
