...
const {appSubUrl, notificationSettings, enableTimeTracking, assetUrlPrefix, sharedWorkerPath} = window.config;
...
export function initStopwatch() {
  ...
  if (notificationSettings.EventSourceUpdateTime > 0 && window.EventSource && window.SharedWorker) {
    ...
    const worker = new SharedWorker(`${assetUrlPrefix}/${sharedWorkerPath}`, 'notification-worker');
