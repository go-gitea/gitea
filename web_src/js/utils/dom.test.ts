...
test('querySingleVisibleElem', () => {
  ...
  el = createElementFromHTML('<div><span class="some-class gt-hidden">foo</span><span>bar</span></div>');
  expect(querySingleVisibleElem(el, 'span')!.textContent).toEqual('bar');
...
test('toggleElem', () => {
  const el = createElementFromHTML('<div><div>a</div><div class="gt-hidden">b</div></div>');
  toggleElem(el.children);
  expect(el.outerHTML).toEqual('<div><div class="gt-hidden">a</div><div class="">b</div></div>');
  toggleElem(el.children, false);
  expect(el.outerHTML).toEqual('<div><div class="gt-hidden">a</div><div class="gt-hidden">b</div></div>');
