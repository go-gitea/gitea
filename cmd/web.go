...
func runHTTPRedirector() {
	...
	if err != nil {
		log.Fatal(0, "Failed to start port redirection: %v", err)
	}
}
...
func runLetsEncrypt(listenAddr, domain, directory, email string, m http.Handler) error {
	...
	go func() {
		...
		if err != nil {
			log.Fatal(0, "Failed to start the Let's Encrypt handler on port %s: %v", setting.PortToRedirect, err)
		}
	}()
	server := &http.Server{
		Addr:    listenAddr,
...
func runWeb(ctx *cli.Context) error {
	...
	case setting.FCGI:
		...
		if err != nil {
			log.Fatal(0, "Failed to bind %s: %v", listenAddr, err)
		}
		defer listener.Close()
		err = fcgi.Serve(listener, context2.ClearHandler(m))
	case setting.UnixSocket:
		if err := os.Remove(listenAddr); err != nil && !os.IsNotExist(err) {
			log.Fatal(0, "Failed to remove unix socket directory %s: %v", listenAddr, err)
		}
		...
		if err = os.Chmod(listenAddr, os.FileMode(setting.UnixSocketPermission)); err != nil {
			log.Fatal(0, "Failed to set permission of unix socket: %v", err)
		}
		err = http.Serve(listener, context2.ClearHandler(m))
	default:
		log.Fatal(0, "Invalid protocol: %s", setting.Protocol)
	}

	if err != nil {
		log.Fatal(0, "Failed to start server: %v", err)
	}
