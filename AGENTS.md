...
- PR descriptions: prose on what and why, no headings, task lists or file listings. Screenshot UI changes, before and after when modifying existing UI
- Never mention the lint, build or test commands you ran
...
- Prefer to avoid database migrations when not necessary
...
- Fix the cause rather than disabling a linter or weakening a test. Where unavoidable, use the narrowest scope with a trailing reason
...
- Aim for sub-2s per integration test and sub-4s per e2e test, waiting on a deterministic condition rather than `sleep`. Prefer semantic locators in e2e tests
