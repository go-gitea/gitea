...
// reciverID > 0 just send to reciver, else send to all watcher
func CreateOrUpdateIssueNotifications(issueID, commentID int64, notificationAuthorID int64, reciverID int64) error {
	sess := x.NewSession()
	defer sess.Close()
	...
	if err := createOrUpdateIssueNotifications(sess, issueID, commentID, notificationAuthorID, reciverID); err != nil {
		return err
	}
...
func createOrUpdateIssueNotifications(e Engine, issueID, commentID int64, notificationAuthorID int64, reciverID int64) error {
	// init
