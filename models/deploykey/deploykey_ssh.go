...
func AddDeployKeySSH(ctx context.Context, repoID int64, name, content string, accessMode perm.AccessMode) (*DeployKey, error) {
	...
	fingerprint, err := asymkey.CalcFingerprint(content)
	if err != nil {
		return nil, err
	}
	var key *DeployKey
	err = globallock.LockAndDo(ctx, asymkey.PublicKeyFingerprintLockKey(fingerprint), func(ctx context.Context) error {
