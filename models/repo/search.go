...
var OrderByMap = map[string]map[string]db.SearchOrderBy{
	"asc": {
		...
		"mirror_updated": "is_mirror DESC, (SELECT `updated_unix` FROM `mirror` WHERE `mirror`.`repo_id` = `repository`.`id`) ASC",
		"size":     "size ASC",
	...
	"desc": {
		...
		"mirror_updated": "is_mirror DESC, (SELECT `updated_unix` FROM `mirror` WHERE `mirror`.`repo_id` = `repository`.`id`) DESC",
		"size":     "size DESC",
