...
func TestReaction(t *testing.T) {
	...
	// Create reaction
	reaction1, err := CreateReaction(t.Context(), &ReactionOptions{
		Type:      "heart",
		DoerID:    user1.ID,
	...
	// Trying to create same reaction again should return ErrReactionAlreadyExist
	...
	assert.Error(t, err)
	assert.True(t, IsErrReactionAlreadyExist(err))

	// Creating reaction with invalid type should return ErrForbiddenReaction
	_, err = CreateReaction(t.Context(), &ReactionOptions{
	...
	assert.Error(t, err)
	assert.True(t, IsErrForbiddenReaction(err))

	// Create second reaction
	reaction2, err := CreateReaction(t.Context(), &ReactionOptions{
		Type:      "laugh",
		DoerID:    user2.ID,
		ReleaseID: release.ID,
	})
	...
	reactions, count, err := FindReactions(t.Context(), FindReactionsOptions{ReleaseID: release.ID})
	assert.NoError(t, err)
	...
	// Delete reaction
	err = DeleteReaction(t.Context(), &ReactionOptions{
		Type:      "heart",
		DoerID:    user1.ID,
		ReleaseID: release.ID,
	})
