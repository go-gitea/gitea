...
func TestRepoAssignees(t *testing.T) {
	...
	users, err := GetRepoAssignees(db.DefaultContext, repo2)
	assert.NoError(t, err)
	assert.Len(t, users, 1)
	assert.Equal(t, users[0].ID, int64(2))

	repo21 := unittest.AssertExistsAndLoadBean(t, &Repository{ID: 21}).(*Repository)
	users, err = GetRepoAssignees(db.DefaultContext, repo21)
	assert.NoError(t, err)
