...
const (
	...
	tplCommand       = "%s --config=%s --work-path=%s serv key-%d"
...
func (key *PublicKey) AuthorizedString() string {
	return fmt.Sprintf(tplPublicKey, util.ShellEscape(fmt.Sprintf(tplCommand, util.ShellEscape(setting.AppPath), util.ShellEscape(setting.CustomConf), util.ShellEscape(setting.AppWorkPath), key.ID)), key.Content)
}
