	...
	if opts.OwnerID > 0 {
		if opts.Starred {
			...
			visibilityCond = builder.Or(
				builder.In("owner_id", builder.Select("org_id").From("org_user").LeftJoin("`user`", builder.Expr("org_user.org_id = \"user\".id")).Where(builder.And(builder.Eq{"uid": opts.OwnerID}, builder.Eq{"visibility": VisibleTypePrivate}))),
