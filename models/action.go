...
func (a *Action) loadActUser() {
	...
	if err == nil {
		...
		log.Error(0, "GetUserByID(%d): %v", a.ActUserID, err)
	}
}
...
func (a *Action) loadRepo() {
	...
	if err != nil {
		log.Error(0, "GetRepositoryByID(%d): %v", a.RepoID, err)
	}
}
...
func (a *Action) GetIssueTitle() string {
	...
	if err != nil {
		log.Error(0, "GetIssueByIndex: %v", err)
...
func CommitRepoAction(opts CommitRepoActionOptions) error {
	...
	if strings.HasPrefix(opts.RefFullName, git.TagPrefix) {
		...
		if err = UpdateIssuesCommit(pusher, repo, opts.Commits.Commits, refName); err != nil {
			log.Error(0, "updateIssuesCommit: %v", err)
		}
	}
	...
	case ActionCommitRepo: // Push
		...
		if isNewBranch {
			...
			if err != nil {
				log.Error(0, "OpenRepository[%s]: %v", repo.RepoPath(), err)
			}

			shaSum, err = gitRepo.GetBranchCommitID(refName)
			if err != nil {
				log.Error(0, "GetBranchCommitID[%s]: %v", opts.RefFullName, err)
			}
	...
	case ActionPushTag: // Create
		...
		if err != nil {
			log.Error(0, "GetTagCommitID[%s]: %v", opts.RefFullName, err)
		}
		if err = PrepareWebhooks(repo, HookEventCreate, &api.CreatePayload{
			Ref:     refName,
			Sha:     shaSum,
