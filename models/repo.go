...
func repoStatsCorrectNumPrivateIssues(ctx context.Context, id int64) error {
	return repoStatsCorrectNumClosed(ctx, id, true, true, "num_private_issues")
}
...
func repoStatsCorrectNumClosedIssues(ctx context.Context, id int64) error {
	return repoStatsCorrectNumClosed(ctx, id, false, false, "num_closed_issues")
}

func repoStatsCorrectNumClosedPulls(ctx context.Context, id int64) error {
	return repoStatsCorrectNumClosed(ctx, id, true, false, "num_closed_pulls")
}

func repoStatsCorrectNumClosed(ctx context.Context, id int64, isPull, isPrivate bool, field string) error {
	_, err := db.GetEngine(ctx).Exec("UPDATE `repository` SET "+field+"=(SELECT COUNT(*) FROM `issue` WHERE repo_id=? AND is_closed=? AND is_pull=? AND is_private=?) WHERE id=?", id, true, isPull, isPrivate, id)
	return err
}
...
func CheckRepoStats(ctx context.Context) error {
	...
	checkers := []*repoChecker{
		...
		{
			statsQuery("SELECT repo.id FROM `repository` repo WHERE repo.num_closed_issues!=(SELECT COUNT(*) FROM `issue` WHERE repo_id=repo.id AND is_closed=? AND is_pull=? and is_private=?)", true, false, false),
...
func UpdateRepoStats(ctx context.Context, id int64) error {
	...
	for _, f := range []func(ctx context.Context, id int64) error{
		...
		repoStatsCorrectNumPrivateIssues,
