...
func (repo *Repository) cloneLink(isWiki bool) *CloneLink {
	...
	if setting.SSH.Port != 22 {
		cl.SSH = fmt.Sprintf("ssh://%s@%s:%d%s/%s/%s.git", setting.RunUser, setting.SSH.Domain, setting.SSH.Port, setting.RepoRootPath, repo.Owner.Name, repoName)
	} else {
		cl.SSH = fmt.Sprintf("%s@%s:%s/%s/%s.git", setting.RunUser, setting.SSH.Domain, setting.RepoRootPath, repo.Owner.Name, repoName)
	}
