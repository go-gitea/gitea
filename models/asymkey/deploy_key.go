...
func checkDeployKeyName(ctx context.Context, repoID int64, name string) error {
	has, err := db.GetEngine(ctx).Where("repo_id=? AND name=?", repoID, name).Get(new(DeployKey))
	if err != nil {
		return err
	} else if has {
...
func checkDeployKey(ctx context.Context, repoID, publicKeyID int64, name string) error {
	...
	has, err := db.GetEngine(ctx).Where("repo_id=? AND key_id=?", repoID, publicKeyID).Get(new(DeployKey))
	if err != nil {
		return err
	} else if has {
...
func addDeployKey(ctx context.Context, repoID, publicKeyID int64, name, fingerprint string, mode perm.AccessMode) (*DeployKey, error) {
	...
	key := &DeployKey{KeyID: publicKeyID, RepoID: repoID, Type: DeployKeyTypeSSH, Name: name, Fingerprint: fingerprint, Mode: mode}
