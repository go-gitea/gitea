...
func TestCreateTaskForRunnerConcurrentClaim(t *testing.T) {
	...
	jobs := make([]*ActionRunJob, numJobs)
	for i := range numJobs {
	...
	runners := make([]*ActionRunner, numJobs)
	for i := range numJobs {
		r := &ActionRunner{
	...
	for i := range numJobs {
		...
		go func(i int) {
			defer wg.Done()
			task, ok, err := CreateTaskForRunner(t.Context(), runners[i])
	...
	for _, j := range jobs {
		updated := unittest.AssertExistsAndLoadBean(t, &ActionRunJob{ID: j.ID})
