...
func (m *Mirror) AfterLoad(session *xorm.Session) {
	...
	if err != nil {
		log.Error(0, "getRepositoryByID[%d]: %v", m.ID, err)
	}
}
...
func (m *Mirror) readAddress() {
	...
	if err != nil {
		log.Error(0, "remoteAddress: %v", err)
	}
}
...
func parseRemoteUpdateOutput(output string) []*mirrorSyncResult {
	...
	for i := range lines {
		...
		case strings.HasPrefix(lines[i], "   "): // New commits of a reference
			...
			if delimIdx == -1 {
				log.Error(0, "SHA delimiter not found: %q", lines[i])
				continue
			}
			shas := strings.Split(lines[i][3:delimIdx+3], "..")
			if len(shas) != 2 {
				log.Error(0, "Expect two SHAs but not what found: %q", lines[i])
				continue
			}
			results = append(results, &mirrorSyncResult{
				refName:     refName,
...
func (m *Mirror) runSync() ([]*mirrorSyncResult, bool) {
	...
	if err != nil {
		...
		if err != nil {
			log.Error(0, "sanitizeOutput: %v", err)
			return nil, false
		}
	...
	if err != nil {
		log.Error(0, "OpenRepository: %v", err)
		return nil, false
	}
	...
	if err := m.Repo.UpdateSize(); err != nil {
		log.Error(0, "Failed to update size for mirror repository: %v", err)
	}
...
func MirrorUpdate() {
	...
	if err := x.
		...
		Iterate(new(Mirror), func(idx int, bean interface{}) error {
			m := bean.(*Mirror)
			if m.Repo == nil {
				log.Error(0, "Disconnected mirror repository found: %d", m.ID)
				return nil
			}
		...
		log.Error(0, "MirrorUpdate: %v", err)
	}
}
...
func SyncMirrors() {
	...
	for repoID := range MirrorQueue.Queue() {
		...
		if err != nil {
			log.Error(0, "GetMirrorByRepoID [%s]: %v", repoID, err)
			continue
		}
		...
		if err = updateMirror(sess, m); err != nil {
			log.Error(0, "UpdateMirror [%s]: %v", repoID, err)
			continue
		}
		...
		if len(results) == 0 {
			...
			if err != nil {
				log.Error(0, "OpenRepository [%d]: %v", m.RepoID, err)
				continue
			}
		}
		...
		for _, result := range results {
			...
			if result.oldCommitID == gitShortEmptySha {
				if err = MirrorSyncCreateAction(m.Repo, result.refName); err != nil {
					log.Error(0, "MirrorSyncCreateAction [repo_id: %d]: %v", m.RepoID, err)
				}
				continue
			}
			...
			if result.newCommitID == gitShortEmptySha {
				if err = MirrorSyncDeleteAction(m.Repo, result.refName); err != nil {
					log.Error(0, "MirrorSyncDeleteAction [repo_id: %d]: %v", m.RepoID, err)
				}
				continue
			}
			...
			if err != nil {
				log.Error(0, "GetFullCommitID [%d]: %v", m.RepoID, err)
				continue
			}
			...
			if err != nil {
				log.Error(0, "CommitsBetweenIDs [repo_id: %d, new_commit_id: %s, old_commit_id: %s]: %v", m.RepoID, newCommitID, oldCommitID, err)
				continue
			}
			if err = MirrorSyncPushAction(m.Repo, MirrorSyncPushActionOptions{
				...
				log.Error(0, "MirrorSyncPushAction [repo_id: %d]: %v", m.RepoID, err)
				continue
			}
		}
