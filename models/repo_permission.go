...
func GetUserRepoPermission(repo *repo_model.Repository, user *user_model.User) (Permission, error) {
	return GetUserRepoPermissionCtx(db.DefaultContext, repo, user)
}

func GetUserRepoPermissionCtx(ctx context.Context, repo *repo_model.Repository, user *user_model.User) (perm Permission, err error) {
	if log.IsTrace() {
		defer func() {
			if user == nil {
