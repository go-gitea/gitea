...
type Permission struct {
	...
	Units      []*RepoUnit
	UnitsMode  map[unit.UnitType]AccessMode
}
...
func (p *Permission) UnitAccessMode(unitType unit.UnitType) AccessMode {
	if p.UnitsMode == nil {
...
func (p *Permission) CanAccess(mode AccessMode, unitType unit.UnitType) bool {
...
func (p *Permission) CanAccessAny(mode AccessMode, unitTypes ...unit.UnitType) bool {
...
func (p *Permission) CanRead(unitType unit.UnitType) bool {
...
func (p *Permission) CanReadAny(unitTypes ...unit.UnitType) bool {
...
func (p *Permission) CanReadIssuesOrPulls(isPull bool) bool {
	if isPull {
		return p.CanRead(unit.UnitTypePullRequests)
	}
	return p.CanRead(unit.UnitTypeIssues)
}

// CanWrite returns true if user could write to this unit
func (p *Permission) CanWrite(unitType unit.UnitType) bool {
...
func (p *Permission) CanWriteIssuesOrPulls(isPull bool) bool {
	if isPull {
		return p.CanWrite(unit.UnitTypePullRequests)
	}
	return p.CanWrite(unit.UnitTypeIssues)
}
...
func getUserRepoPermission(e db.Engine, repo *Repository, user *User) (perm Permission, err error) {
	...
	perm.UnitsMode = make(map[unit.UnitType]AccessMode)
...
func AccessLevel(user *User, repo *Repository) (AccessMode, error) {
	return accessLevelUnit(db.GetEngine(db.DefaultContext), user, repo, unit.UnitTypeCode)
}

// AccessLevelUnit returns the Access a user has to a repository's. Will return NoneAccess if the
// user does not have access.
func AccessLevelUnit(user *User, repo *Repository, unitType unit.UnitType) (AccessMode, error) {
	return accessLevelUnit(db.GetEngine(db.DefaultContext), user, repo, unitType)
}

func accessLevelUnit(e db.Engine, user *User, repo *Repository, unitType unit.UnitType) (AccessMode, error) {
...
func hasAccessUnit(e db.Engine, user *User, repo *Repository, unitType unit.UnitType, testMode AccessMode) (bool, error) {
...
func HasAccessUnit(user *User, repo *Repository, unitType unit.UnitType, testMode AccessMode) (bool, error) {
...
func canBeAssigned(e db.Engine, user *User, repo *Repository, _ bool) (bool, error) {
	...
	return perm.CanAccessAny(AccessModeWrite, unit.UnitTypeCode, unit.UnitTypeIssues, unit.UnitTypePullRequests), nil
}
...
func FilterOutRepoIdsWithoutUnitAccess(u *User, repoIDs []int64, units ...unit.UnitType) ([]int64, error) {
	i := 0
