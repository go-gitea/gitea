...
func TestMilestone_State(t *testing.T) {
	assert.Equal(t, api.StateOpen, (&milestone_model.Milestone{IsClosed: false}).State())
	assert.Equal(t, api.StateClosed, (&milestone_model.Milestone{IsClosed: true}).State())
}

func TestGetMilestoneByRepoID(t *testing.T) {
	assert.NoError(t, unittest.PrepareTestDatabase())

	milestone, err := milestone_model.GetMilestoneByRepoID(db.DefaultContext, 1, 1)
	assert.NoError(t, err)
	assert.EqualValues(t, 1, milestone.ID)
	assert.EqualValues(t, 1, milestone.RepoID)

	_, err = milestone_model.GetMilestoneByRepoID(db.DefaultContext, unittest.NonexistentID, unittest.NonexistentID)
	assert.True(t, milestone_model.IsErrMilestoneNotExist(err))
}
...
func TestGetMilestones(t *testing.T) {
	...
	test := func(sortType string, sortCond func(*milestone_model.Milestone) int) {
	...
	test("furthestduedate", func(milestone *milestone_model.Milestone) int {
		return -int(milestone.DeadlineUnix)
	})
	test("leastcomplete", func(milestone *milestone_model.Milestone) int {
		return milestone.Completeness
	})
	test("mostcomplete", func(milestone *milestone_model.Milestone) int {
		return -milestone.Completeness
	})
	test("leastissues", func(milestone *milestone_model.Milestone) int {
		return milestone.NumIssues
	})
	test("mostissues", func(milestone *milestone_model.Milestone) int {
		return -milestone.NumIssues
	})
	test("soonestduedate", func(milestone *milestone_model.Milestone) int {
		return int(milestone.DeadlineUnix)
	})
}
...
func TestCountRepoMilestones(t *testing.T) {
	...
	test := func(repoID int64) {
		repo := unittest.AssertExistsAndLoadBean(t, &repo_model.Repository{ID: repoID})
		count, err := db.Count[milestone_model.Milestone](db.DefaultContext, milestone_model.FindMilestoneOptions{
			RepoID: repoID,
		})
		assert.NoError(t, err)
...
func TestCountMilestonesByRepoIDs(t *testing.T) {
	...
	openCounts, err := milestone_model.CountMilestonesMap(db.DefaultContext, milestone_model.FindMilestoneOptions{
		RepoIDs:  []int64{1, 2},
		IsClosed: util.OptionalBoolFalse,
	})
	assert.NoError(t, err)
	...
	closedCounts, err := milestone_model.CountMilestonesMap(db.DefaultContext,
		milestone_model.FindMilestoneOptions{
			RepoIDs:  []int64{1, 2},
			IsClosed: util.OptionalBoolTrue,
		})
	assert.NoError(t, err)
...
func TestGetMilestonesByRepoIDs(t *testing.T) {
	...
	test := func(sortType string, sortCond func(*milestone_model.Milestone) int) {
		for _, page := range []int{0, 1} {
			openMilestones, err := db.Find[milestone_model.Milestone](db.DefaultContext, milestone_model.FindMilestoneOptions{
				ListOptions: db.ListOptions{
					Page:     page,
					PageSize: setting.UI.IssuePagingNum,
				},
			...
			closedMilestones, err := db.Find[milestone_model.Milestone](db.DefaultContext,
				milestone_model.FindMilestoneOptions{
					ListOptions: db.ListOptions{
						Page:     page,
						PageSize: setting.UI.IssuePagingNum,
...
func TestNewMilestone(t *testing.T) {
	...
	assert.NoError(t, milestone_model.NewMilestone(db.DefaultContext, milestone))
	unittest.AssertExistsAndLoadBean(t, milestone)
	unittest.CheckConsistencyFor(t, &repo_model.Repository{ID: milestone.RepoID}, &milestone_model.Milestone{})
}
...
func TestChangeMilestoneStatus(t *testing.T) {
	...
	assert.NoError(t, milestone_model.ChangeMilestoneStatus(db.DefaultContext, milestone, true))
	unittest.AssertExistsAndLoadBean(t, &milestone_model.Milestone{ID: 1}, "is_closed=1")
	unittest.CheckConsistencyFor(t, &repo_model.Repository{ID: milestone.RepoID}, &milestone_model.Milestone{})

	assert.NoError(t, milestone_model.ChangeMilestoneStatus(db.DefaultContext, milestone, false))
	unittest.AssertExistsAndLoadBean(t, &milestone_model.Milestone{ID: 1}, "is_closed=0")
	unittest.CheckConsistencyFor(t, &repo_model.Repository{ID: milestone.RepoID}, &milestone_model.Milestone{})
}

func TestDeleteMilestoneByRepoID(t *testing.T) {
	assert.NoError(t, unittest.PrepareTestDatabase())
	assert.NoError(t, milestone_model.DeleteMilestoneByRepoID(db.DefaultContext, 1, 1))
	unittest.AssertNotExistsBean(t, &milestone_model.Milestone{ID: 1})
	unittest.CheckConsistencyFor(t, &repo_model.Repository{ID: 1})

	assert.NoError(t, milestone_model.DeleteMilestoneByRepoID(db.DefaultContext, unittest.NonexistentID, unittest.NonexistentID))
}
...
func TestUpdateMilestone(t *testing.T) {
	...
	assert.NoError(t, milestone_model.UpdateMilestone(db.DefaultContext, milestone, milestone.IsClosed))
	milestone = unittest.AssertExistsAndLoadBean(t, &milestone_model.Milestone{ID: 1})
	assert.EqualValues(t, "newMilestoneName", milestone.Name)
	unittest.CheckConsistencyFor(t, &milestone_model.Milestone{})
}
...
func TestUpdateMilestoneCounters(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.NoError(t, milestone_model.UpdateMilestoneCounters(db.DefaultContext, issue.MilestoneID))
	unittest.CheckConsistencyFor(t, &milestone_model.Milestone{})
...
func TestMigrate_InsertMilestones(t *testing.T) {
	...
	err := milestone_model.InsertMilestones(db.DefaultContext, ms)
	assert.NoError(t, err)
