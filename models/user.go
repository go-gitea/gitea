...
func (opts *SearchUserOptions) toConds() builder.Cond {
	...
	if opts.OwnerID > 0 {
		...
		accessCond = builder.Or(
			builder.In("id", builder.Select("org_id").From("org_user").Join("INNER", "user", "org_user.org_id = user.id").Where(builder.And(builder.Eq{"uid": opts.OwnerID}, builder.Eq{"visibility": VisibleTypePrivate}))),
