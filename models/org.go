...
// FromUser converts user to organization
func FromUser(user *user_model.User) *Organization {
...
// FindTeamsCtx returns all temas of an organization
func FindTeamsCtx(ctx context.Context, orgID int64) ([]*Team, error) {
	var teams []*Team
...
func CreateOrganization(org *Organization, owner *user_model.User) (err error) {
	...
	if err != nil {
		...
		return user_model.ErrUserAlreadyExist{org.Name}
	}
	...
	if err = (*user_model.User)(org).GenerateRandomAvatarCtx(ctx); err != nil {
...
func DeleteOrganization(org *Organization) (err error) {
	if !(*user_model.User)(org).IsOrganization() {
	...
	if err = deleteOrg(ctx, org); err != nil {
...
func deleteOrg(ctx context.Context, u *Organization) error {
	e := db.GetEngine(ctx)
	...
	if _, err = e.ID(u.ID).NoAutoCondition().Delete(u); err != nil {
	...
	if len(u.Avatar) > 0 {
		avatarPath := (*user_model.User)(u).CustomAvatarRelativePath()
...
func (u *Organization) HasMemberWithUserID(userID int64) bool {
	return u.hasMemberWithUserID(db.GetEngine(db.DefaultContext), userID)
}

func (u *Organization) hasMemberWithUserID(e db.Engine, userID int64) bool {
...
type MinimalOrg = user_model.User
...
func getOwnedOrgsByUserID(ctx context.Context, userID int64, desc string) ([]*Organization, error) {
	orgs := make([]*Organization, 0, 10)
...
func HasOrgOrUserVisible(org *Organization, user *user_model.User) bool {
	return hasOrgOrUserVisible(db.GetEngine(db.DefaultContext), org, user)
}

func hasOrgOrUserVisible(e db.Engine, orgOrUser *Organization, user *user_model.User) bool {
	// Not SignedUser
	if user == nil {
...
func GetOwnedOrgsByUserID(userID int64) ([]*Organization, error) {
	return getOwnedOrgsByUserID(db.DefaultContext, userID, "")
}
...
func GetOwnedOrgsByUserIDDesc(userID int64, desc string) ([]*Organization, error) {
	return getOwnedOrgsByUserID(db.DefaultContext, userID, desc)
}
...
// GetOrgByIDCtx get organization by id
func GetOrgByIDCtx(ctx context.Context, orgID int64) (*Organization, error) {
	user, err := user_model.GetUserByIDCtx(ctx, orgID)
	if err != nil {
		return nil, err
	}
...
func removeOrgUser(ctx context.Context, orgID, userID int64) error {
	...
	env, err := org.accessibleReposEnv(ctx, userID)
	if err != nil {
...
func (org *Organization) AccessibleReposEnv(userID int64) (AccessibleReposEnvironment, error) {
	return org.accessibleReposEnv(db.DefaultContext, userID)
}

func (org *Organization) accessibleReposEnv(ctx context.Context, userID int64) (AccessibleReposEnvironment, error) {
