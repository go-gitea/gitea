...
func GetActivityStats(ctx context.Context, repo *repo_model.Repository, timeFrom, timeUntil time.Time, releases, issues, prs, code bool) (*ActivityStats, error) {
	stats := &ActivityStats{Code: &git.CodeActivityStats{}}
	if releases {
		if err := stats.FillReleases(ctx, repo.ID, timeFrom, timeUntil); err != nil {
			return nil, fmt.Errorf("FillReleases: %w", err)
		}
	}
	if prs {
		if err := stats.FillPullRequests(ctx, repo.ID, timeFrom, timeUntil); err != nil {
			return nil, fmt.Errorf("FillPullRequests: %w", err)
		}
	}
	if issues {
		if err := stats.FillIssues(ctx, repo.ID, timeFrom, timeUntil); err != nil {
			return nil, fmt.Errorf("FillIssues: %w", err)
		}
	}
	if err := stats.FillUnresolvedIssues(ctx, repo.ID, timeFrom, timeUntil, issues, prs); err != nil {
	...
	if code {
		...
		code, err := gitRepo.GetCodeActivityStats(timeFrom, timeUntil, repo.DefaultBranch)
		if err != nil {
...
func GetActivityStatsTopAuthors(ctx context.Context, repo *repo_model.Repository, timeFrom, timeUntil time.Time, count int) ([]*ActivityAuthorData, error) {
	gitRepo, closer, err := gitrepo.RepositoryFromContextOrOpen(ctx, repo)
	if err != nil {
		return nil, fmt.Errorf("OpenRepository: %w", err)
	}
	...
	code, err := gitRepo.GetCodeActivityStats(timeFrom, timeUntil, "")
	if err != nil {
		return nil, fmt.Errorf("FillFromGit: %w", err)
	}
	if code.Authors == nil {
...
func (stats *ActivityStats) FillPullRequests(ctx context.Context, repoID int64, fromTime, untilTime time.Time) error {
	var err error
	var count int64

	// Merged pull requests
	sess := pullRequestsForActivityStatement(ctx, repoID, fromTime, untilTime, true)
	...
	sess = pullRequestsForActivityStatement(ctx, repoID, fromTime, untilTime, false)
...
func pullRequestsForActivityStatement(ctx context.Context, repoID int64, fromTime, untilTime time.Time, merged bool) db.Session {
	...
	if merged {
		...
		sess.And("pull_request.merged_unix <= ?", untilTime.Unix())
	} else {
		sess.And("issue.is_closed = ?", false)
		sess.And("issue.created_unix >= ?", fromTime.Unix())
		sess.And("issue.created_unix <= ?", untilTime.Unix())
	}
...
func (stats *ActivityStats) FillIssues(ctx context.Context, repoID int64, fromTime, untilTime time.Time) error {
	var err error
	var count int64

	// Closed issues
	sess := issuesForActivityStatement(ctx, repoID, fromTime, untilTime, true, false)
	...
	// New issues
	sess = newlyCreatedIssues(ctx, repoID, fromTime, untilTime)
	sess.OrderBy("issue.created_unix ASC")
	...
	// Active issues
	sess = activeIssues(ctx, repoID, fromTime, untilTime)
	sess.OrderBy("issue.created_unix ASC")
	...
	sess = issuesForActivityStatement(ctx, repoID, fromTime, untilTime, false, false)
	if _, err = sess.Select("count(distinct issue.poster_id) as `count`").Table("issue").Get(&count); err != nil {
		return err
	}
...
func (stats *ActivityStats) FillUnresolvedIssues(ctx context.Context, repoID int64, fromTime, untilTime time.Time, issues, prs bool) error {
	// Check if we need to select anything
	if !issues && !prs {
		return nil
	}
	sess := issuesForActivityStatement(ctx, repoID, fromTime, untilTime, false, true)
	if !issues || !prs {
...
func newlyCreatedIssues(ctx context.Context, repoID int64, fromTime, untilTime time.Time) db.Session {
	sess := db.GetEngine(ctx).Where("issue.repo_id = ?", repoID).
		...
		And("issue.created_unix <= ?", untilTime.Unix())

	return sess
}

func activeIssues(ctx context.Context, repoID int64, fromTime, untilTime time.Time) db.Session {
	sess := db.GetEngine(ctx).Where("issue.repo_id = ?", repoID).
		And("issue.is_pull = ?", false).
		And(builder.Or(
			builder.And(
				builder.Gte{"issue.created_unix": fromTime.Unix()},
				builder.Lte{"issue.created_unix": untilTime.Unix()},
			),
			builder.And(
				builder.Gte{"issue.closed_unix": fromTime.Unix()},
				builder.Lte{"issue.closed_unix": untilTime.Unix()},
			),
		))
...
func issuesForActivityStatement(ctx context.Context, repoID int64, fromTime, untilTime time.Time, closed, unresolved bool) db.Session {
	sess := db.GetEngine(ctx).Where("issue.repo_id = ?", repoID).
	...
	if !unresolved {
		...
		if closed {
			...
			sess.And("issue.closed_unix <= ?", untilTime.Unix())
		} else {
			sess.And("issue.created_unix >= ?", fromTime.Unix())
			sess.And("issue.created_unix <= ?", untilTime.Unix())
		}
		...
		sess.And("issue.updated_unix <= ?", untilTime.Unix())
	}
...
func (stats *ActivityStats) FillReleases(ctx context.Context, repoID int64, fromTime, untilTime time.Time) error {
	var err error
	var count int64

	// Published releases list
	sess := releasesForActivityStatement(ctx, repoID, fromTime, untilTime)
...
func releasesForActivityStatement(ctx context.Context, repoID int64, fromTime, untilTime time.Time) db.Session {
	return db.GetEngine(ctx).Where("`release`.repo_id = ?", repoID).
		And("`release`.is_draft = ?", false).
		And("`release`.created_unix >= ?", fromTime.Unix()).
		And("`release`.created_unix <= ?", untilTime.Unix())
}
