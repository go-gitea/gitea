...
func (protectBranch *ProtectedBranch) IsUserOfficialReviewer(user *user_model.User) (bool, error) {
	return protectBranch.isUserOfficialReviewer(db.DefaultContext, user)
}

func (protectBranch *ProtectedBranch) isUserOfficialReviewer(ctx context.Context, user *user_model.User) (bool, error) {
	e := db.GetEngine(ctx)
	...
	if !protectBranch.EnableApprovalsWhitelist {
		...
		writeAccess, err := hasAccessUnit(ctx, user, repo, unit.TypeCode, AccessModeWrite)
		if err != nil {
			return false, err
		}
