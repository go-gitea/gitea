...
func (pr *PullRequest) GetDefaultMergeMessage() string {
	if pr.HeadRepo == nil {
		...
		if err != nil {
			log.Error(0, "GetRepositoryById[%d]: %v", pr.HeadRepoID, err)
			return ""
		}
	}
...
func (pr *PullRequest) apiFormat(e Engine) *api.PullRequest {
	...
	if err = pr.Issue.loadRepo(e); err != nil {
		log.Error(0, "loadRepo[%d]: %v", pr.ID, err)
		return nil
	}
	...
	if pr.BaseRepo == nil {
		...
		if err != nil {
			log.Error(0, "GetRepositoryById[%d]: %v", pr.ID, err)
			return nil
		}
	}
	if pr.HeadRepo == nil {
...
func (pr *PullRequest) Merge(doer *User, baseGitRepo *git.Repository, mergeStyle MergeStyle, message string) (err error) {
	...
	if err = pr.setMerged(); err != nil {
		log.Error(0, "setMerged [%d]: %v", pr.ID, err)
	}

	if err = MergePullRequestAction(doer, pr.Issue.Repo, pr.Issue); err != nil {
		log.Error(0, "MergePullRequestAction [%d]: %v", pr.ID, err)
	}
	...
	if err != nil {
		log.Error(0, "CommitsBetweenIDs: %v", err)
		return nil
	}
...
func (pr *PullRequest) manuallyMerged() bool {
	...
	if err != nil {
		log.Error(0, "PullRequest[%d].getMergeCommit: %v", pr.ID, err)
		return false
	}
	if commit != nil {
		...
		if merger == nil {
			if pr.BaseRepo.Owner == nil {
				if err = pr.BaseRepo.getOwner(x); err != nil {
					log.Error(0, "BaseRepo.getOwner[%d]: %v", pr.ID, err)
					return false
				}
			}
		...
		if err = pr.setMerged(); err != nil {
			log.Error(0, "PullRequest[%d].setMerged : %v", pr.ID, err)
			return false
		}
...
func addHeadRepoTasks(prs []*PullRequest) {
	for _, pr := range prs {
		...
		if err := pr.UpdatePatch(); err != nil {
			log.Error(0, "UpdatePatch: %v", err)
			continue
		} else if err := pr.PushToBaseRepo(); err != nil {
			log.Error(0, "PushToBaseRepo: %v", err)
			continue
		}
...
func AddTestPullRequestTask(doer *User, repoID int64, branch string, isSync bool) {
	...
	if err != nil {
		log.Error(0, "Find pull requests [head_repo_id: %d, head_branch: %s]: %v", repoID, branch, err)
		return
	}
	...
	if isSync {
		...
		if err = requests.LoadAttributes(); err != nil {
			log.Error(0, "PullRequestList.LoadAttributes: %v", err)
		}
		if invalidationErr := checkForInvalidation(requests, repoID, doer, branch); invalidationErr != nil {
			log.Error(0, "checkForInvalidation: %v", invalidationErr)
		}
		if err == nil {
			for _, pr := range prs {
				...
				if err = PrepareWebhooks(pr.Issue.Repo, HookEventPullRequest, &api.PullRequestPayload{
					...
					log.Error(0, "PrepareWebhooks [pull_id: %v]: %v", pr.ID, err)
					continue
				}
				go HookQueue.Add(pr.Issue.Repo.ID)
			}
	...
	if err != nil {
		log.Error(0, "Find pull requests [base_repo_id: %d, base_branch: %s]: %v", repoID, branch, err)
		return
	}
	for _, pr := range prs {
		pr.AddToTaskQueue()
...
func checkForInvalidation(requests PullRequestList, repoID int64, doer *User, branch string) error {
	...
	go func() {
		...
		if err != nil {
			log.Error(0, "PullRequestList.InvalidateCodeComments: %v", err)
		}
	}()
	return nil
}
...
func (pr *PullRequest) checkAndUpdateStatus() {
	...
	if !pullRequestQueue.Exist(pr.ID) {
		if err := pr.UpdateCols("status, conflicted_files"); err != nil {
			log.Error(0, "Update[%d]: %v", pr.ID, err)
		}
	}
}
...
func TestPullRequests() {
	...
	for prID := range pullRequestQueue.Queue() {
		...
		if err != nil {
			log.Error(0, "GetPullRequestByID[%s]: %v", prID, err)
			continue
		} else if pr.manuallyMerged() {
			continue
		} else if err = pr.testPatch(x); err != nil {
			log.Error(0, "testPatch[%d]: %v", pr.ID, err)
			continue
		}
