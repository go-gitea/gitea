...
func renameExistingUserAvatarName(x *xorm.Engine) error {
	...
	for {
		...
		for _, user := range users {
			...
			if stat, err := os.Stat(filepath.Join(setting.Avatar.UploadPath, oldAvatar)); err != nil || !stat.Mode().IsRegular() {
				if err == nil {
			...
			deleteList[filepath.Join(setting.Avatar.UploadPath, oldAvatar)] = struct{}{}
			migrated++
			select {
			case <-ticker.C:
				log.Info(
...
func copyOldAvatarToNewLocation(userID int64, oldAvatar string) (string, error) {
	fr, err := os.Open(filepath.Join(setting.Avatar.UploadPath, oldAvatar))
	if err != nil {
	...
	if err := ioutil.WriteFile(filepath.Join(setting.Avatar.UploadPath, newAvatar), data, 0666); err != nil {
