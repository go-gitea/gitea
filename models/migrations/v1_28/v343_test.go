...
type packageBlobUploadAfterV343 struct {
	ID            string `xorm:"pk"`
	CreatorID     int64  `xorm:"INDEX NOT NULL DEFAULT 0"`
	BytesReceived int64  `xorm:"NOT NULL DEFAULT 0"`
}

func (packageBlobUploadAfterV343) TableName() string {
	return "package_blob_upload"
}

func Test_AddCreatorIDToPackageBlobUpload(t *testing.T) {
	x, deferable := migrationtest.PrepareTestEnv(t, 0, new(packageBlobUploadBeforeV343))
	defer deferable()

	_, err := x.Insert(&packageBlobUploadBeforeV343{
		ID:            "test-upload",
		BytesReceived: 12,
	})
	...
	var pbu packageBlobUploadAfterV343
