...
func Format(format string, args ...any) (msg string, err error) {
	...
	for _, arg := range args {
		...
		if val.Kind() == reflect.Slice {
			// Previously, we would accept TrString(lang, key, a, [b, c], d, [e, f]) as Sprintf(msg, a, b, c, d, e, f)
			// but this is an unstable behavior.
			//
			// So we restrict the accepted arguments to either:
			//
			// 1. TrString(lang, key, [slice-items]) as Sprintf(msg, items...)
			// 2. TrString(lang, key, args...) as Sprintf(msg, args...)
			if len(args) == 1 {
				for i := 0; i < val.Len(); i++ {
