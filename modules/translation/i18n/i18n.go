...
func (l *locale) tryTr(trKey string, trArgs ...interface{}) (msg string, found bool) {
	...
	return l.format(trKey, trMsg, trArgs...), found
}

func (l *locale) format(trKey, trMsg string, trArgs ...interface{}) string {
	if len(trArgs) == 0 {
		return trMsg
	}
...
func (l *locale) HasMessage(key string) bool {
	...
	textIdx, ok := l.store.idxForTrKey(key)
	if !ok {
		return false
	}

	_, found := l.idxToMsgMap[textIdx]
	return found
}
...
func (l *locale) TrPlurals(cnt interface{}, ruleType, key string, args ...interface{}) string {
	...
	if form == plurals.Other {
		return l.Tr(key, args...) // Fall back to the standard
	}
	...
	pluralKey := key + "_" + string(form)

	if textIdx, ok := l.store.idxForTrKey(pluralKey); ok {
		if msg, found := l.idxToMsgMap[textIdx]; found {
			return l.format(key, msg, args...)
		}
	}

	stdMsg, _ := l.tryTr(key, args...) // fall back to the standard
	return stdMsg
}
