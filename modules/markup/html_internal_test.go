...
func externalIssueLink(baseURL, class, name string) string {
	return link(util.URLJoinDeprecated(baseURL, name), class, name)
}

// numericLink an HTML to a numeric-style issue
func numericIssueLink(baseURL, class string, index int, marker string) string {
	return link(util.URLJoinDeprecated(baseURL, strconv.Itoa(index)), class, fmt.Sprintf("%s%d", marker, index))
}
...
func TestRender_IssueIndexPattern2(t *testing.T) {
	...
	test := func(s, expectedFmt, marker string, indices ...int) {
		...
		for i, index := range indices {
			links[i] = numericIssueLink(util.URLJoinDeprecated("/test-owner/test-repo", path), "ref-issue", index, marker)
		}
...
func TestRender_IssueIndexPattern5(t *testing.T) {
	...
	test := func(s, expectedFmt, pattern string, ids, names []string) {
		...
		for i, id := range ids {
			links[i] = link(util.URLJoinDeprecated("https://someurl.com/someUser/someRepo/", id), "ref-issue ref-external-issue", names[i])
		}
...
func TestRender_AutoLink(t *testing.T) {
	...
	test(util.URLJoinDeprecated(TestRepoURL, "issues", "3333"),
		numericIssueLink(util.URLJoinDeprecated(TestRepoURL, "issues"), "ref-issue", 3333, "#"))

	// render valid commit URLs
	tmp := util.URLJoinDeprecated(TestRepoURL, "commit", "d8a994ef243349f321568f9e36d5c3f444b99cae")
