...
func (p *Parser) Render(rawBytes []byte, urlPrefix string, metas map[string]string, isWiki bool) []byte {
	...
	if p.IsInputFile {
		...
		if err != nil {
			log.Error(0, "%s create temp file when rendering %s failed: %v", p.Name(), p.Command, err)
			return []byte("")
		}
		...
		if err != nil {
			f.Close()
			log.Error(0, "%s write data to temp file when rendering %s failed: %v", p.Name(), p.Command, err)
			return []byte("")
		}

		err = f.Close()
		if err != nil {
			log.Error(0, "%s close temp file when rendering %s failed: %v", p.Name(), p.Command, err)
			return []byte("")
		}
	...
	if err := cmd.Run(); err != nil {
		log.Error(0, "%s render run command %s %v failed: %v", p.Name(), commands[0], args, err)
		return []byte("")
	}
	return buf.Bytes()
}
