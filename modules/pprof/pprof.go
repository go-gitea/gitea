...
func DumpMemProfileForUsername(pprofDataPath, username string) {
	...
	if err != nil {
		log.GitLogger.Fatal(0, "Could not create memory profile: %v", err)
	}
	defer f.Close()
	runtime.GC() // get up-to-date statistics
	if err := pprof.WriteHeapProfile(f); err != nil {
		log.GitLogger.Fatal(0, "Could not write memory profile: %v", err)
	}
}
...
func DumpCPUProfileForUsername(pprofDataPath, username string) func() {
	...
	if err != nil {
		log.GitLogger.Fatal(0, "Could not create cpu profile: %v", err)
	}
