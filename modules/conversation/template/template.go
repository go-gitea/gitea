...
// Validate checks whether an ConversationTemplate is considered valid, and returns the first error
func Validate(template *api.ConversationTemplate) error {
	if err := validateMetadata(template); err != nil {
		return err
	}
	if template.Type() == api.ConversationTemplateTypeYaml {
...
func validateMetadata(template *api.ConversationTemplate) error {
...
func validateYaml(template *api.ConversationTemplate) error {
	...
	for idx, field := range template.Fields {
		...
		switch field.Type {
		case api.ConversationFormFieldTypeMarkdown:
			if err := validateStringItem(position, field.Attributes, true, "value"); err != nil {
				return err
			}
		case api.ConversationFormFieldTypeTextarea:
		...
		case api.ConversationFormFieldTypeInput:
			if err := validateStringItem(position, field.Attributes, false,
				"description",
				"placeholder",
				"value",
		...
		case api.ConversationFormFieldTypeDropdown:
		...
		case api.ConversationFormFieldTypeCheckboxes:
			if err := validateStringItem(position, field.Attributes, false, "description"); err != nil {
				return err
			}
			if err := validateOptions(field, idx); err != nil {
...
func validateLabel(field *api.ConversationFormField, idx int) error {
	if field.Type == api.ConversationFormFieldTypeMarkdown {
...
func validateRequired(field *api.ConversationFormField, idx int) error {
	if field.Type == api.ConversationFormFieldTypeMarkdown || field.Type == api.ConversationFormFieldTypeCheckboxes {
...
func validateID(field *api.ConversationFormField, idx int, ids container.Set[string]) error {
	if field.Type == api.ConversationFormFieldTypeMarkdown {
...
func validateOptions(field *api.ConversationFormField, idx int) error {
	if field.Type != api.ConversationFormFieldTypeDropdown && field.Type != api.ConversationFormFieldTypeCheckboxes {
		return nil
	}
	position := newErrorPosition(idx, field.Type)
...
func newErrorPosition(fieldIdx int, fieldType api.ConversationFormFieldType, optionIndex ...int) errorPosition {
...
func RenderToMarkdown(template *api.ConversationTemplate, values url.Values) string {
	builder := &strings.Builder{}

	for _, field := range template.Fields {
		f := &valuedField{
			ConversationFormField: field,
			Values:                values,
		}
...
func (f *valuedField) HideLabel() bool {
	if f.Type == api.ConversationFormFieldTypeMarkdown {
		return true
	}
...
func (o *valuedOption) VisibleInContent() bool {
	if o.field.Type == api.ConversationFormFieldTypeCheckboxes {
		if vs, ok := o.data.(map[string]any); ok {
