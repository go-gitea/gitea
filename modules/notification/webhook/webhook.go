...
func (m *webhookNotifier) NotifyIssueClearLabels(doer *models.User, issue *models.Issue) {
	...
	if issue.IsPull {
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequestLabel, &api.PullRequestPayload{
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssueLabel, &api.IssuePayload{
			Action:     api.HookIssueLabelCleared,
			Index:      issue.Index,
...
func (m *webhookNotifier) NotifyForkRepository(doer *models.User, oldRepo, repo *models.Repository) {
	...
	// forked webhook
	if err := webhook_services.PrepareWebhooks(oldRepo, nil, models.HookEventFork, &api.ForkPayload{
	...
	if u.IsOrganization() {
		if err := webhook_services.PrepareWebhooks(repo, nil, models.HookEventRepository, &api.RepositoryPayload{
...
func (m *webhookNotifier) NotifyIssueChangeAssignee(doer *models.User, issue *models.Issue, assignee *models.User, removed bool, comment *models.Comment) {
	if issue.IsPull {
		...
		if err := webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequestAssign, apiPullRequest); err != nil {
		...
		// Assignee comment triggers a webhook
		if err := webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssueAssign, apiIssue); err != nil {
			log.Error("PrepareWebhooks [is_pull: %v, remove_assignee: %v]: %v", issue.IsPull, removed, err)
			return
		}
	}
...
func (m *webhookNotifier) NotifyIssueChangeTitle(doer *models.User, issue *models.Issue, oldTitle string) {
	...
	if issue.IsPull {
		...
		issue.PullRequest.Issue = issue
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequest, &api.PullRequestPayload{
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssues, &api.IssuePayload{
			Action: api.HookIssueEdited,
			Index:  issue.Index,
			Changes: &api.ChangesPayload{
				Title: &api.ChangesFromPayload{
...
func (m *webhookNotifier) NotifyIssueChangeStatus(doer *models.User, issue *models.Issue, actionComment *models.Comment, isClosed bool) {
	...
	if issue.IsPull {
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequest, apiPullRequest)
	} else {
		apiIssue := &api.IssuePayload{
			Index:      issue.Index,
			Issue:      convert.ToAPIIssue(issue),
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssues, apiIssue)
	}
	if err != nil {
...
func (m *webhookNotifier) NotifyNewIssue(issue *models.Issue, mentions []*models.User) {
	...
	mode, _ := models.AccessLevel(issue.Poster, issue.Repo)
	if err := webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssues, &api.IssuePayload{
...
func (m *webhookNotifier) NotifyNewPullRequest(pull *models.PullRequest, mentions []*models.User) {
	...
	if err := webhook_services.PrepareWebhooks(pull.Issue.Repo, nil, models.HookEventPullRequest, &api.PullRequestPayload{
		Action:      api.HookIssueOpened,
...
func (m *webhookNotifier) NotifyUpdateComment(doer *models.User, c *models.Comment, oldContent string) {
	...
	if c.Issue.IsPull {
		err = webhook_services.PrepareWebhooks(c.Issue.Repo, nil, models.HookEventPullRequestComment, &api.IssueCommentPayload{
		...
		err = webhook_services.PrepareWebhooks(c.Issue.Repo, nil, models.HookEventIssueComment, &api.IssueCommentPayload{
			Action:  api.HookIssueCommentEdited,
			Issue:   convert.ToAPIIssue(c.Issue),
			Comment: convert.ToComment(c),
			Changes: &api.ChangesPayload{
...
func (m *webhookNotifier) NotifyCreateIssueComment(doer *models.User, repo *models.Repository,
	...
	var err error
	if issue.IsPull {
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequestComment, &api.IssueCommentPayload{
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssueComment, &api.IssueCommentPayload{
			Action:     api.HookIssueCommentCreated,
			Issue:      convert.ToAPIIssue(issue),
			Comment:    convert.ToComment(comment),
			Repository: convert.ToRepo(repo, mode),
...
func (m *webhookNotifier) NotifyDeleteComment(doer *models.User, comment *models.Comment) {
	...
	if comment.Issue.IsPull {
		err = webhook_services.PrepareWebhooks(comment.Issue.Repo, nil, models.HookEventPullRequestComment, &api.IssueCommentPayload{
		...
		err = webhook_services.PrepareWebhooks(comment.Issue.Repo, nil, models.HookEventIssueComment, &api.IssueCommentPayload{
			Action:     api.HookIssueCommentDeleted,
			Issue:      convert.ToAPIIssue(comment.Issue),
			Comment:    convert.ToComment(comment),
			Repository: convert.ToRepo(comment.Issue.Repo, mode),
...
func (m *webhookNotifier) NotifyIssueChangeMilestone(doer *models.User, issue *models.Issue, oldMilestoneID int64) {
	...
	if issue.IsPull {
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventPullRequestMilestone, &api.PullRequestPayload{
			Action:      hookAction,
			Index:       issue.Index,
			PullRequest: convert.ToAPIPullRequest(issue.PullRequest),
			Repository:  convert.ToRepo(issue.Repo, mode),
		...
		err = webhook_services.PrepareWebhooks(issue.Repo, nil, models.HookEventIssueMilestone, &api.IssuePayload{
			Action:     hookAction,
			Index:      issue.Index,
			Issue:      convert.ToAPIIssue(issue),
			Repository: convert.ToRepo(issue.Repo, mode),
...
func (m *webhookNotifier) NotifyPushCommits(pusher *models.User, repo *models.Repository, opts *repository.PushUpdateOptions, commits *repository.PushCommits) {
	...
	if err := webhook_services.PrepareWebhooks(repo, nil, models.HookEventPush, &api.PushPayload{
...
func (*webhookNotifier) NotifyMergePullRequest(pr *models.PullRequest, doer *models.User) {
	...
	err = webhook_services.PrepareWebhooks(pr.Issue.Repo, nil, models.HookEventPullRequest, apiPullRequest)
	if err != nil {
		log.Error("PrepareWebhooks: %v", err)
	}
}
...
func (m *webhookNotifier) NotifyPullRequestReview(pr *models.PullRequest, review *models.Review, comment *models.Comment, mentions []*models.User) {
	...
	if err := webhook_services.PrepareWebhooks(review.Issue.Repo, nil, reviewHookType, &api.PullRequestPayload{
...
func (m *webhookNotifier) NotifyCreateRef(pusher *models.User, repo *models.Repository, refType, refFullName string) {
	...
	if err = webhook_services.PrepareWebhooks(repo, nil, models.HookEventCreate, &api.CreatePayload{
		Ref:     refName,
		Sha:     shaSum,
		RefType: refType,
		Repo:    apiRepo,
...
func (m *webhookNotifier) NotifyPullRequestSynchronized(doer *models.User, pr *models.PullRequest) {
	...
	if err := webhook_services.PrepareWebhooks(pr.Issue.Repo, nil, models.HookEventPullRequestSync, &api.PullRequestPayload{
...
func (m *webhookNotifier) NotifyDeleteRef(pusher *models.User, repo *models.Repository, refType, refFullName string) {
	...
	if err := webhook_services.PrepareWebhooks(repo, nil, models.HookEventDelete, &api.DeletePayload{
		Ref:        refName,
		RefType:    refType,
...
func sendReleaseHook(doer *models.User, rel *models.Release, action api.HookReleaseAction) {
	...
	if err := webhook_services.PrepareWebhooks(rel.Repo, nil, models.HookEventRelease, &api.ReleasePayload{
		Action:     action,
...
func (*webhookNotifier) NotifyAddOrgTeam(doer, org *models.User, team *models.Team) {
	...
	if err := webhook_services.PrepareWebhooks(nil, org, models.HookEventTeam, &api.TeamPayload{
...
func (*webhookNotifier) NotifyAddTeamMember(doer, org, member *models.User, team *models.Team) {
	...
	if err := webhook_services.PrepareWebhooks(nil, org, models.HookEventTeamMember, &api.MembershipPayload{
