...
func (q *QueueNotifier) handle(data ...queue.Data) {
	for _, datum := range data {
		...
		case "NotifyMergePullRequest":
			...
			var unknown2 *git.Repository
			err = json.Unmarshal(call.Args[2], &unknown2)
			if err != nil {
				log.Error("Unable to unmarshal %s to %s in call to %s: %v", string(call.Args[2]), "*git.Repository", "NotifyMergePullRequest", err)
				continue
			}
			for _, notifier := range q.notifiers {
				notifier.NotifyMergePullRequest(unknown0, unknown1, unknown2)
			}
		...
		case "NotifyPushCommits":
			...
			if err != nil {
				log.Error("Unable to unmarshal %s to %s in call to %s: %v", string(call.Args[5]), "*models.PushCommits", "NotifyPushCommits", err)
				continue
			}
			for _, notifier := range q.notifiers {
		...
		case "NotifySyncPushCommits":
			...
			err = json.Unmarshal(call.Args[5], &commits)
			if err != nil {
				log.Error("Unable to unmarshal %s to %s in call to %s: %v", string(call.Args[5]), "*models.PushCommits", "NotifySyncPushCommits", err)
				continue
			}
			for _, notifier := range q.notifiers {
...
func (q *QueueNotifier) NotifyMergePullRequest(unknown0 *models.PullRequest, unknown1 *models.User, unknown2 *git.Repository) {
	args := make([][]byte, 0)
	var err error
	var bs []byte
	bs, err = json.Marshal(&unknown0)
...
func (q *QueueNotifier) NotifyPushCommits(pusher *models.User, repo *models.Repository, refName string, oldCommitID string, newCommitID string, commits *models.PushCommits) {
	args := make([][]byte, 0)
	var err error
	var bs []byte
...
func (q *QueueNotifier) NotifySyncPushCommits(pusher *models.User, repo *models.Repository, refName string, oldCommitID string, newCommitID string, commits *models.PushCommits) {
	args := make([][]byte, 0)
	var err error
	var bs []byte
	bs, err = json.Marshal(&pusher)
