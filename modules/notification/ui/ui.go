...
func (ns *notificationService) handle(data ...queue.Data) {
	for _, datum := range data {
		...
		if err := models.CreateOrUpdateIssueNotifications(opts.IssueID, opts.CommentID, opts.NotificationAuthorID, opts.ReciverID); err != nil {
...
func (ns *notificationService) NotifyPullRewiewRequest(doer *models.User, issue *models.Issue, reviewer *models.User, isRequest bool) {
	if isRequest {
		var opts = issueNotificationOpts{
			IssueID:              issue.ID,
			NotificationAuthorID: doer.ID,
			ReciverID:            reviewer.ID,
		}
