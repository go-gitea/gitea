...
func getRefName(ctx *Base, repo *Repository, pathType RepoRefType) string {
	...
	case RepoRefLegacy, RepoRefAny:
		...
		parts := strings.Split(path, "/")
		if len(parts) > 0 && len(parts[0]) == repo.GitRepo.HashType.FullLength() {
	...
	case RepoRefCommit:
		parts := strings.Split(path, "/")
		if len(parts) > 0 && len(parts[0]) >= 7 && len(parts[0]) <= repo.GitRepo.HashType.FullLength() {
			repo.TreePath = strings.Join(parts[1:], "/")
			return parts[0]
		}
	...
	return func(ctx *Context) (cancel context.CancelFunc) {
		...
		if len(ctx.Params("*")) == 0 {
			...
			if refType.RefTypeIncludesBranches() && ctx.Repo.GitRepo.IsBranchExist(refName) {
				...
				ctx.Repo.CommitID = ctx.Repo.Commit.ID.String()
			} else if len(refName) >= 7 && len(refName) <= ctx.Repo.GitRepo.HashType.FullLength() {
				...
				if len(refName) < ctx.Repo.GitRepo.HashType.FullLength() {
