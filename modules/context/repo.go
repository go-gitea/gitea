...
func RepoAssignment(ctx *Context) context.CancelFunc {
	...
	gitRepo, err := git.OpenRepository(ctx, storage.RepoPath(userName, repoName))
	if err != nil {
		...
		ctx.ServerError("RepoAssignment Invalid repo "+storage.RepoPath(userName, repoName), err)
		return nil
	}
...
func RepoRefByType(refType RepoRefType, ignoreNotExistErr ...bool) func(*Context) context.CancelFunc {
	return func(ctx *Context) (cancel context.CancelFunc) {
		...
		if ctx.Repo.GitRepo == nil {
			repoPath := storage.RepoPath(ctx.Repo.Owner.Name, ctx.Repo.Repository.Name)
