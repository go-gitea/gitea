...
func NewFuncMap() []template.FuncMap {
	return []template.FuncMap{map[string]interface{}{
		...
		"svg": func(icon string, size int) template.HTML {
			return template.HTML(fmt.Sprintf(`<svg class="svg %s" width="%d" height="%d"><use xlink:href="%s/img/svg/icons.svg#%s" /></svg>`, icon, size, size, setting.StaticURLPrefix, icon))
		},
	}}
}
