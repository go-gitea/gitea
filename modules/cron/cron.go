...
func NewContext() {
	...
	if setting.Cron.UpdateMirror.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Update mirrors]: %v", err)
		}
	...
	if setting.Cron.RepoHealthCheck.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Repository health check]: %v", err)
		}
	...
	if setting.Cron.CheckRepoStats.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Check repository statistics]: %v", err)
		}
	...
	if setting.Cron.ArchiveCleanup.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Clean up old repository archives]: %v", err)
		}
	...
	if setting.Cron.SyncExternalUsers.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Synchronize external users]: %v", err)
		}
	...
	if setting.Cron.DeletedBranchesCleanup.Enabled {
		...
		if err != nil {
			log.Fatal(0, "Cron[Remove old deleted branches]: %v", err)
		}
