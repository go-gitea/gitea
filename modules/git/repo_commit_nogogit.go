...
func (repo *Repository) ConvertToHash(commitID string) (Hash, error) {
	if len(commitID) == repo.HashType.FullLength() && repo.HashType.IsValid(commitID) {
		hash, err := NewIDFromString(commitID)
		if err == nil {
			return hash, nil
		}
	}
