...
func (repo *Repository) CreateTag(name, revision string) error {
	_, _, err := gitcmd.New("tag").AddDashesAndList(name, revision).RunStdString(repo.Ctx, &gitcmd.RunOpts{Dir: repo.Path})
	return err
}

// CreateAnnotatedTag create one annotated tag in the repository
func (repo *Repository) CreateAnnotatedTag(name, message, revision string) error {
	_, _, err := gitcmd.New("tag", "-a", "-m").AddDynamicArguments(message).AddDashesAndList(name, revision).RunStdString(repo.Ctx, &gitcmd.RunOpts{Dir: repo.Path})
	return err
}
...
func (repo *Repository) GetTagNameBySHA(sha string) (string, error) {
	...
	stdout, _, err := gitcmd.New("show-ref", "--tags", "-d").RunStdString(repo.Ctx, &gitcmd.RunOpts{Dir: repo.Path})
	if err != nil {
		return "", err
	}
...
func (repo *Repository) GetTagID(name string) (string, error) {
	stdout, _, err := gitcmd.New("show-ref", "--tags").AddDashesAndList(name).RunStdString(repo.Ctx, &gitcmd.RunOpts{Dir: repo.Path})
	if err != nil {
		return "", err
	}
