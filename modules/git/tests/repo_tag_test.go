...
func TestRepository_GetTag(t *testing.T) {
	RunTestPerProvider(t, func(service service.GitService, t *testing.T) {
		...
		bareRepo1, err := service.OpenRepository(clonedPath)
		assert.NoError(t, err)
		defer bareRepo1.Close()
		...
		assert.EqualValues(t, lTagCommitID, lTag.TagObject().String())
		...
		assert.NotEqual(t, aTagID, aTag.TagObject().String())
		assert.EqualValues(t, aTagCommitID, aTag.TagObject().String())
