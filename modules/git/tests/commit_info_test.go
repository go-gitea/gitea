...
func testGetCommitsInfo(t *testing.T, gitService service.GitService, repo1 service.Repository) {
	...
	for _, testCase := range testCases {
		...
		assert.NotNil(t, commit.Repository())
		...
		assert.NotNil(t, tree.Repository(), "repo is nil for testCase CommitID %s in Path %s", testCase.CommitID, testCase.Path)

		assert.NoError(t, err)
		entries, err := tree.ListEntries()
		assert.NoError(t, err)
		commitsInfo, treeCommit, err := gitService.GetCommitsInfo(commit, testCase.Path, entries, nil)
		assert.NoError(t, err)
		if err != nil {
			t.FailNow()
		}
...
func TestEntries_GetCommitsInfo(t *testing.T) {
	RunTestPerProvider(t, func(gitService service.GitService, t *testing.T) {
		...
		bareRepo1, err := gitService.OpenRepository(bareRepo1Path)
		assert.NoError(t, err)
		defer bareRepo1.Close()

		testGetCommitsInfo(t, gitService, bareRepo1)

		clonedPath, err := cloneRepo(bareRepo1Path, testReposDir, "repo1_TestEntries_GetCommitsInfo")
		assert.NoError(t, err)
		defer util.RemoveAll(clonedPath)
		clonedRepo1, err := gitService.OpenRepository(clonedPath)
		assert.NoError(t, err)
		defer clonedRepo1.Close()

		testGetCommitsInfo(t, gitService, clonedRepo1)
...
func BenchmarkEntries_GetCommitsInfo(b *testing.B) {
	...
	RunBenchmarkPerProvider(b, func(gitService service.GitService, b *testing.B) {
		for _, benchmark := range benchmarks {
			...
			if repoPath, err := cloneRepo(benchmark.url, benchmarkReposDir, benchmark.name); err != nil {
				b.Fatal(err)
			} else if repo, err = gitService.OpenRepository(repoPath); err != nil {
				b.Fatal(err)
			...
			b.Run(benchmark.name, func(b *testing.B) {
				for i := 0; i < b.N; i++ {
					_, _, err := gitService.GetCommitsInfo(commit, "", entries, nil)
					if err != nil {
						b.Fatal(err)
					}
				}
