...
func (CommitsInfoService) NewLastCommitCache(gitRepo service.Repository, ttl int64, cache service.Cache) service.LastCommitCache {
	...
	return &LastCommitCache{
		repo:        gitRepo,
		commitCache: make(map[string]service.Commit),
		ttl:         ttl,
		cache:       cache,
	}
}
...
func (csi CommitsInfoService) GetCommitsInfo(commit service.Commit, treePath string, tes service.Entries, cache service.LastCommitCache) ([]service.CommitInfo, service.Commit, error) {
	...
	var revs map[string]service.Commit
	if cache != nil {
		...
		revs = map[string]service.Commit{}
		var foundCommits []service.Commit
	...
	if treePath == "" {
		...
		log.Warn("Unable to find tree commit for %q @ %s in %s", treePath, commit.ID().String(), commit.Repository().Path())
	}
	return commitsInfo, treeCommit, nil
}

func getLastCommitForPathsByCache(commitID, treePath string, paths []string, cache service.LastCommitCache) (map[string]service.Commit, []string, error) {
	var unHitEntryPaths []string
	var results = make(map[string]service.Commit)
	for _, p := range paths {
...
func GetLastCommitForPaths(commit service.Commit, treePath string, paths []string) ([]service.Commit, error) {
	...
	go common.PipeCommand(
		git.NewCommand("rev-list", "--format=%T", commit.ID().String()),
...
revListLoop:
	...
	commitsMap := make(map[string]service.Commit, len(commits))
	commitsMap[commit.ID().String()] = commit

	commitCommits := make([]service.Commit, len(commits))
	for i, commitID := range commits {
		...
		c, err = CommitFromReader(commit.Repository(), StringHash(string(commitID)), io.LimitReader(batchReader, int64(size)))
		if err != nil {
			return nil, err
		}
		commitCommits[i] = c
