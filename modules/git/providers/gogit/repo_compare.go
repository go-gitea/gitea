...
func (repo *Repository) GetCompareInfo(basePath, baseBranch, headBranch string) (_ *service.CompareInfo, err error) {
	...
	if err == nil {
		...
		logs, err := git.NewCommand("log", compareInfo.MergeBase+"..."+headBranch, native.LogHashFormat).RunInDirBytes(repo.Path())
		if err != nil {
			return nil, err
		}
		compareInfo.Commits, err = native.BatchReadCommits(repo, bytes.NewReader(logs))
		if err != nil {
