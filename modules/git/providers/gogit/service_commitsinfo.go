...
var _ (service.CommitsInfoService) = CommitsInfoService{}

// CommitsInfoService represents a service that provides commits info
type CommitsInfoService struct {
}

// NewLastCommitCache creates a new last commit cache for repo
func (CommitsInfoService) NewLastCommitCache(gitRepo service.Repository, ttl int64, cache service.Cache) service.LastCommitCache {
	if cache == nil {
		return nil
	}
...
func (CommitsInfoService) GetCommitsInfo(commit service.Commit, treePath string, tes service.Entries, cache service.LastCommitCache) ([]service.CommitInfo, service.Commit, error) {
	...
	c, err := commitNodeIndex.Get(ToPlumbingHash(commit.ID()))
	if err != nil {
		return nil, nil, err
	}
	...
	commitsInfo := make([]service.CommitInfo, len(tes))
	for i, entry := range tes {
		commitsInfo[i] = service.CommitInfo{
			Entry: entry,
		}
		if rev, ok := revs[entry.Name()]; ok {
			entryCommit := convertCommit(repo, rev)
			...
			if entry.Mode().IsSubModule() {
				...
				subModuleFile := common.NewSubModuleFile(entryCommit, subModuleURL, entry.ID().String())
	...
	var treeCommit service.Commit
	if treePath == "" {
		treeCommit = commit
	} else if rev, ok := revs[""]; ok {
		treeCommit = convertCommit(repo, rev)
	}
...
func getLastCommitForPathsByCache(commitID, treePath string, paths []string, cache service.LastCommitCache) (map[string]*object.Commit, []string, error) {
