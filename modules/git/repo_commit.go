...
func (repo *Repository) searchCommits(ctx context.Context, id ObjectID, opts SearchCommitsOptions) ([]*Commit, error) {
	...
	for _, v := range opts.Keywords {
		// ignore anything not matching a valid sha pattern (TODO: objectIdMinLen is pretty small, is it right or fine?)
		if IsStringValidObjectID(id.Type(), v, objectIdMinLen) {
