...
func (repo *Repository) GetDefaultPublicGPGKey(forceUpdate bool) (*GPGSettings, error) {
	...
	value, _, _ := NewCommand(repo.Ctx, "config", "--get", "commit.gpgsign").RunWithContextString(&RunContext{Dir: repo.Path})
	...
	signingKey, _, _ := NewCommand(repo.Ctx, "config", "--get", "user.signingkey").RunWithContextString(&RunContext{Dir: repo.Path})
	gpgSettings.KeyID = strings.TrimSpace(signingKey)

	defaultEmail, _, _ := NewCommand(repo.Ctx, "config", "--get", "user.email").RunWithContextString(&RunContext{Dir: repo.Path})
	gpgSettings.Email = strings.TrimSpace(defaultEmail)

	defaultName, _, _ := NewCommand(repo.Ctx, "config", "--get", "user.name").RunWithContextString(&RunContext{Dir: repo.Path})
