...
func TestAddTemplateSubmoduleIndexes(t *testing.T) {
	...
	tmpDir := t.TempDir()
	var err error
	_, _, err = gitcmd.New("init").RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpDir})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	_, _, err = gitcmd.New("add", "--all").RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpDir})
	require.NoError(t, err)
	_, _, err = gitcmd.New("-c", "user.name=a", "-c", "user.email=b", "commit", "-m=test").RunStdString(ctx, &gitcmd.RunOpts{Dir: tmpDir})
	require.NoError(t, err)
