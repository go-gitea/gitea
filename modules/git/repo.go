...
func (repo *Repository) CreateBundle(ctx context.Context, commit string, out io.Writer) error {
	...
	_, _, err = gitcmd.NewCommand("init", "--bare").WithEnv(env).WithDir(tmp).RunStdString(ctx)
	if err != nil {
		return err
	}

	_, _, err = gitcmd.NewCommand("reset", "--soft").AddDynamicArguments(commit).WithEnv(env).WithDir(tmp).RunStdString(ctx)
	if err != nil {
		return err
	}

	_, _, err = gitcmd.NewCommand("branch", "-m", "bundle").WithEnv(env).WithDir(tmp).RunStdString(ctx)
	if err != nil {
		return err
	}

	tmpFile := filepath.Join(tmp, "bundle")
	_, _, err = gitcmd.NewCommand("bundle", "create").AddDynamicArguments(tmpFile, "bundle", "HEAD").WithEnv(env).WithDir(tmp).RunStdString(ctx)
	if err != nil {
		return err
	}
