...
func AddChangesWithArgs(repoPath string, globalArgs []string, all bool, files ...string) error {
	...
	cmd.AddArguments("--")
	_, _, err := cmd.AddArguments(files...).RunWithContextString(&RunContext{Dir: repoPath})
	return err
}
...
func CommitChangesWithArgs(repoPath string, args []string, opts CommitChangesOptions) error {
	...
	_, _, err := cmd.RunWithContextString(&RunContext{Dir: repoPath})
...
func AllCommitsCount(ctx context.Context, repoPath string, hidePRRefs bool, files ...string) (int64, error) {
	...
	stdout, _, err := cmd.RunWithContextString(&RunContext{Dir: repoPath})
	if err != nil {
		return 0, err
	}
...
func (c *Commit) HasPreviousCommit(commitHash SHA1) (bool, error) {
	...
	if err := CheckGitVersionAtLeast("1.8"); err == nil {
		_, _, err := NewCommand(c.repo.Ctx, "merge-base", "--is-ancestor", that, this).RunWithContextString(&RunContext{Dir: c.repo.Path})
		if err == nil {
			return true, nil
		}
	...
	result, _, err := NewCommand(c.repo.Ctx, "rev-list", "--ancestry-path", "-n1", that+".."+this, "--").RunWithContextString(&RunContext{Dir: c.repo.Path})
	if err != nil {
		return false, err
	}
...
func (c *Commit) GetBranchName() (string, error) {
	...
	data, _, err := NewCommand(c.repo.Ctx, args...).RunWithContextString(&RunContext{Dir: c.repo.Path})
	if err != nil {
...
func (c *Commit) GetTagName() (string, error) {
	data, _, err := NewCommand(c.repo.Ctx, "describe", "--exact-match", "--tags", "--always", c.ID.String()).RunWithContextString(&RunContext{Dir: c.repo.Path})
	if err != nil {
...
func GetFullCommitID(ctx context.Context, repoPath, shortID string) (string, error) {
	commitID, _, err := NewCommand(ctx, "rev-parse", shortID).RunWithContextString(&RunContext{Dir: repoPath})
	if err != nil {
