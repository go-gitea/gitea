...
func LoadRepoConfig() {
	...
	for i, t := range types {
		...
		if isDir {
			...
			for _, f := range customFiles {
				if !util.SliceContainsString(f, files, true) {
	...
	for _, name := range setting.Repository.PreferredLicenses {
		if util.SliceContainsString(name, Licenses, true) {
			sortedLicenses = append(sortedLicenses, name)
		}
	}
	for _, name := range Licenses {
		if !util.SliceContainsString(name, setting.Repository.PreferredLicenses, true) {
			sortedLicenses = append(sortedLicenses, name)
		}
	}
