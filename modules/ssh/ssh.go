...
func publicKeyHandler(ctx ssh.Context, key ssh.PublicKey) bool {
	...
	principalLoop:
		for _, principal := range cert.ValidPrincipals {
			pkey, err := keys_model.SearchPublicKeyByContentExact(principal)
			if err != nil {
	...
	pkey, err := keys_model.SearchPublicKeyByContent(strings.TrimSpace(string(gossh.MarshalAuthorizedKey(key))))
	if err != nil {
		if keys_model.IsErrKeyNotExist(err) {
			if log.IsWarn() {
