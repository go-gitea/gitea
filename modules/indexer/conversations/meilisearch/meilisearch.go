...
const (
	conversationIndexerLatestVersion = 3
...
func NewIndexer(url, apiKey, indexerName string) *Indexer {
	...
	inner := inner_meilisearch.NewIndexer(url, apiKey, indexerName, conversationIndexerLatestVersion, settings)
	indexer := &Indexer{
		inner:   inner,
		Indexer: inner,
	}
...
func (b *Indexer) Index(_ context.Context, conversations ...*internal.IndexerData) error {
	...
	for _, conversation := range conversations {
		_, err := b.inner.Client.Index(b.inner.VersionedIndexName()).AddDocuments(conversation)
		if err != nil {
			return err
		}
	}
...
func convertHits(searchRes *meilisearch.SearchResponse) ([]internal.Match, error) {
	...
	for _, hit := range searchRes.Hits {
		...
		conversationID, ok := hit["id"].(float64)
		if !ok {
			return nil, ErrMalformedResponse
		}
