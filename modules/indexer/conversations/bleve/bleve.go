...
const (
	conversationIndexerAnalyzer      = "conversationIndexer"
	conversationIndexerDocType       = "conversationIndexerDocType"
	conversationIndexerLatestVersion = 4
)
...
// IndexerData an update to the conversation indexer
type IndexerData internal.IndexerData

// Type returns the document type, for bleve's mapping.Classifier interface.
func (i *IndexerData) Type() string {
	return conversationIndexerDocType
}

// generateConversationIndexMapping generates the bleve index mapping for conversations
func generateConversationIndexMapping() (mapping.IndexMapping, error) {
	...
	if err := addUnicodeNormalizeTokenFilter(mapping); err != nil {
		return nil, err
	} else if err = mapping.AddCustomAnalyzer(conversationIndexerAnalyzer, map[string]any{
		"type":          custom.Name,
		"char_filters":  []string{},
	...
	mapping.DefaultAnalyzer = conversationIndexerAnalyzer
	mapping.AddDocumentMapping(conversationIndexerDocType, docMapping)
...
func NewIndexer(indexDir string) *Indexer {
	inner := inner_bleve.NewIndexer(indexDir, conversationIndexerLatestVersion, generateConversationIndexMapping)
	return &Indexer{
		Indexer: inner,
		inner:   inner,
	}
...
func (b *Indexer) Index(_ context.Context, conversations ...*internal.IndexerData) error {
	batch := inner_bleve.NewFlushingBatch(b.inner.Indexer, maxBatchSize)
	for _, conversation := range conversations {
		if err := batch.Index(indexer_internal.Base36(conversation.ID), (*IndexerData)(conversation)); err != nil {
			return err
		}
	}
	return batch.Flush()
...
func (b *Indexer) Search(ctx context.Context, options *internal.SearchOptions) (*internal.SearchResult, error) {
	...
	if options.Keyword != "" {
		...
		queries = append(queries, bleve.NewDisjunctionQuery([]query.Query{
			inner_bleve.MatchPhraseQuery(options.Keyword, "title", conversationIndexerAnalyzer, fuzziness),
			inner_bleve.MatchPhraseQuery(options.Keyword, "content", conversationIndexerAnalyzer, fuzziness),
			inner_bleve.MatchPhraseQuery(options.Keyword, "comments", conversationIndexerAnalyzer, fuzziness),
		}...))
	}
