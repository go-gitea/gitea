...
import (
	...
	"gitea.dev/modules/indexer/internal/zoekt/meta"
...
func (i *Indexer) Init(_ context.Context) (bool, error) {
	...
	metadata, found, err := meta.ReadIndexMetadata(i.indexDir)
	if err != nil {
		return false, fmt.Errorf("failed to read index metadata: %w", err)
	}
	if found && metadata.Version != i.version {
		compare := "older"
	...
	if !found || metadata.Version != i.version {
		// the indexer is using a previous version, or the directory does not hold
		// an index of ours at all, so remove whatever is in it and re-populate
		if exists {
			if err = util.RemoveAllWithRetry(i.indexDir); err != nil {
				return false, err
			}
			exists = false
