...
func getRepoChanges(ctx context.Context, repo *repo_model.Repository, revision string) (*base.RepoChanges, error) {
...
func parseGitLsTreeOutput(stdout []byte) ([]base.FileUpdate, error) {
	entries, err := git.ParseTreeEntries(stdout)
	if err != nil {
		return nil, err
	}
	idxCount := 0
	updates := make([]base.FileUpdate, len(entries))
	for _, entry := range entries {
		if isIndexable(entry) {
			updates[idxCount] = base.FileUpdate{
				Filename: entry.Name(),
...
func genesisChanges(ctx context.Context, repo *repo_model.Repository, revision string) (*base.RepoChanges, error) {
	var changes base.RepoChanges
...
func nonGenesisChanges(ctx context.Context, repo *repo_model.Repository, revision string) (*base.RepoChanges, error) {
	...
	if runErr != nil {
		...
		if err := holder.Get().(base.Indexer).Delete(repo.ID); err != nil {
			return nil, err
		}
		return genesisChanges(ctx, repo, revision)
	}
