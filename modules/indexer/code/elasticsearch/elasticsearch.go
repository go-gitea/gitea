...
func (b *Indexer) addUpdate(ctx context.Context, batch *git.BatchCatFile, sha string, update internal.FileUpdate, repo *repo_model.Repository) ([]elastic.BulkableRequest, error) {
	...
	if err := batch.Input(update.BlobSha); err != nil {
		return nil, err
	}
...
func (b *Indexer) Index(ctx context.Context, repo *repo_model.Repository, sha string, changes *internal.RepoChanges) error {
	...
	if len(changes.Updates) > 0 {
		batch, err := git.NewBatchCatFile(ctx, repo.RepoPath(), false)
		if err != nil {
			return err
		}
		defer batch.Close()
