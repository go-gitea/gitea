...
func (b *Indexer) addUpdate(ctx context.Context, builder *build.Builder, batchWriter git.WriteCloserError, batchReader *bufio.Reader, update internal.FileUpdate, repo *repo_model.Repository) error {
...
func (b *Indexer) Delete(_ context.Context, repoID int64) error {
	...
	pattern := fmt.Sprintf("%d_v*.00000.zoekt*", repoID)
	...
	for _, filePath := range matches {
		if err := os.Remove(filePath); err != nil {
			// 如果删除过程中遇到错误，记录错误但继续尝试删除其他文件
			log.Error("failed to delete %s: %v", filePath, err)
		}
	}

	tmpPattern := fmt.Sprintf("%d_v*.tmp", repoID)
...
func (b *Indexer) generateZoektQuery(_ context.Context, opts *internal.SearchOptions) (query.Q, error) {
	...
	case indexer.SearchModeWords:
		...
		// Process first word
		contentQuery, err = query.Parse(TransToZoektContentQueryString(fields[0]))
		if err != nil {
		...
		for _, field := range fields[1:] {
			fieldQuery, err := query.Parse(TransToZoektContentQueryString(field))
			if err != nil {
...
func (b *Indexer) Search(ctx context.Context, opts *internal.SearchOptions) (int64, []*internal.SearchResult, []*internal.SearchResultLanguages, error) {
	...
	for _, item := range result.Files {
		item.LineMatches = slices.DeleteFunc(item.LineMatches, func(line zoekt.LineMatch) bool {
