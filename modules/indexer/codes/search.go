...
func searchResult(result *Match, startIndex, endIndex int) (*Result, error) {
...
func PerformSearch(repoIDs []int64, keyword string, page, pageSize int) (int, []*Result, error) {
	...
	result, err := codesIndexer.Search(repoIDs, keyword, page, pageSize)
	if err != nil {
		return 0, nil, err
	}

	displayResults := make([]*Result, len(result.Hits))

	for i, hit := range result.Hits {
		startIndex, endIndex := indices(hit.Content, hit.StartIndex, hit.EndIndex)
		displayResults[i], err = searchResult(&hit, startIndex, endIndex)
		if err != nil {
			return 0, nil, err
		}
	}
	return int(result.Total), displayResults, nil
}
