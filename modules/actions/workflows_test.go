...
func TestDetectMatched(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:           "HookEventPullRequestReviewComment(pull_request_review_comment) matches GithubEventPullRequestReviewComment(pull_request_review_comment) with `created` activity type",
		...
		{
			desc:           "HookEventRelease(release) `updated` action matches GithubEventRelease(release) with `edited` activity type",
			triggeredEvent: webhook_module.HookEventRelease,
		...
		{
			desc:           "HookEventRelease(release) `updated` action matches GithubEventRelease(release)",
			triggeredEvent: webhook_module.HookEventRelease,
			payload:        &api.ReleasePayload{Action: api.HookReleaseUpdated},
			yamlOn:         "on:\n  release:",
			expected:       true,
		...
		{
			desc:           "HookEventPackage(package) `created` action matches GithubEventRegistryPackage(registry_package)",
			triggeredEvent: webhook_module.HookEventPackage,
			payload:        &api.PackagePayload{Action: api.HookPackageCreated},
			yamlOn:         "on:\n  registry_package:",
			expected:       true,
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			evts, err := jobparser.GetEventsFromContent([]byte(tc.yamlOn + minJob))
