...
func (gdrive *Gdrive) cachePut(path string, node *drive.File) error {
	if cc := cache.GetCache(); cc != nil {
		...
		return cc.Put(fmt.Sprintf("gdrive:%s:%s", gdrive.rootDrive.Id, gdrive.fixPath(path)), string(body), 60*24)
	}
	return nil
}
...
func (gdrive *Gdrive) GetRequest(node *drive.FilesGetCall) (*http.Response, error) {
	...
	for i := 0; i < 10 && err != nil; i++ {
		if urlError, ok := err.(*url.Error); ok {
			...
			log.Error("gdrive error: typeof original error %s, error %s", reflect.TypeOf(urlError.Err).String(), urlError.Err.Error())
		}
		log.Error("gdrive error: typeof error %s, error %s", reflect.TypeOf(err).String(), err.Error())
		break
	}
	return res, err
}
...
func (gdrive *Gdrive) Open(path string) (Object, error) {
	...
	return &driveOpen{fileNode, gdrive, boot, 0}, nil
}
...
func (gdrive *Gdrive) IterateObjects(path string, iterator func(path string, obj Object) error) (err error) {
	...
	if current, err = gdrive.GetNode(path); err != nil {
		...
		return iterator(gdrive.fixPath(path), &driveOpen{current, gdrive, nil, 0})
	}
	...
	recursiveCall = func(path, folderID string) error {
		...
		for _, k := range files {
			...
			} else if err := iterator(newPath, &driveOpen{k, gdrive, nil, 0}); err != nil {
				return err
			}
		}
		return nil
