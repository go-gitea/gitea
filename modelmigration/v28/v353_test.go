...
func (secretBeforeV353) TableName() string { return "secret" }

type actionVariableBeforeV353 struct {
	ID          int64              `xorm:"pk autoincr"`
...
func (actionVariableBeforeV353) TableName() string { return "action_variable" }

type actionRunJobBeforeV353 struct {
	ID     int64 `xorm:"pk autoincr"`
	RepoID int64 `xorm:"index"`
}

func (actionRunJobBeforeV353) TableName() string { return "action_run_job" }

type secretV353 struct {
	ID            int64              `xorm:"pk autoincr"`
	OwnerID       int64              `xorm:"INDEX UNIQUE(owner_repo_name) NOT NULL"`
	RepoID        int64              `xorm:"INDEX UNIQUE(owner_repo_name) NOT NULL DEFAULT 0"`
...
func (secretV353) TableName() string { return "secret" }

func Test_AddActionEnvironmentSchema(t *testing.T) {
	x, deferable := migrationtest.PrepareTestEnv(t, 0,
		new(secretBeforeV353),
		new(actionVariableBeforeV353),
		new(actionRunJobBeforeV353),
	)
	defer deferable()
	if x == nil || t.Failed() {
		return
	...
	_, err := x.Insert(&secretBeforeV353{OwnerID: 1, Name: "TOKEN", Data: "secret-data"})
	...
	_, err = x.Insert(&secretV353{OwnerID: 1, EnvironmentID: 1, Name: "TOKEN", Data: "env-scoped"})
	require.NoError(t, err)
	_, err = x.Insert(&secretV353{OwnerID: 1, EnvironmentID: 1, Name: "TOKEN", Data: "dup"})
	require.Error(t, err)
}
