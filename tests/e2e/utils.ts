...
export async function apiCreateFiles(requestContext: APIRequestContext, owner: string, repo: string, files: Array<{path: string; content: string; operation?: string}>, {branch, newBranch, headers}: {branch?: string; newBranch?: string; headers?: Record<string, string>} = {}) {
  await apiRetry(() => requestContext.post(`${baseUrl()}/api/v1/repos/${owner}/${repo}/contents`, {
    headers: headers || apiHeaders(),
    data: {
      branch, new_branch: newBranch,
      files: files.map((file) => ({operation: file.operation ?? 'create', path: file.path, content: Buffer.from(file.content, 'utf8').toString('base64')})),
    },
  }), 'apiCreateFiles');
}
