...
test('external render file displays in iframe', async ({page, request}) => {
  ...
  try {
    ...
    // verify rendered content appears inside the iframe
...
test('openapi file renders swagger in iframe', async ({page, request}) => {
  ...
  try {
    ...
    // verify SwaggerUI renders inside the iframe
    const frame = page.frameLocator('iframe.external-render-iframe');
