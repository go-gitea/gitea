...
func TestRepoCommitsStatusMultiple(t *testing.T) {
	...
	doc = NewHTMLParser(t, resp.Body)
	// Check that the data-global-init="commit-statuses" (for trigger) and commit-status (svg) are present
	sel := doc.doc.Find("#commits-table .message [data-global-init=\"commit-statuses\"] .commit-status")
	assert.Equal(t, 1, sel.Length())
}
