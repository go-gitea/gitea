...
func TestAPIGitTags(t *testing.T) {
	...
	_ = git.NewCommand(git.DefaultContext, "config", "user.name").AddUntrustedArguments(user.Name).Run(&git.RunOpts{Dir: repo.RepoPath()})
	_ = git.NewCommand(git.DefaultContext, "config", "user.email").AddUntrustedArguments(user.Email).Run(&git.RunOpts{Dir: repo.RepoPath()})
