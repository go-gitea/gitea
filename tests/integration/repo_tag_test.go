...
func TestCreateNewTagProtected(t *testing.T) {
	...
	t.Run("Git", func(t *testing.T) {
		onGiteaRun(t, func(t *testing.T, u *url.URL) {
			...
			_, _, err := gitcmd.New("tag", "v-2").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			assert.NoError(t, err)

			_, _, err = gitcmd.New("push", "--tags").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			assert.Error(t, err)
	...
	t.Run("GitTagForce", func(t *testing.T) {
		onGiteaRun(t, func(t *testing.T, u *url.URL) {
			...
			_, _, err := gitcmd.New("tag", "v-1.1", "-m", "force update", "--force").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			require.NoError(t, err)

			_, _, err = gitcmd.New("push", "--tags").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			require.NoError(t, err)

			_, _, err = gitcmd.New("tag", "v-1.1", "-m", "force update v2", "--force").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			require.NoError(t, err)
			...
			_, _, err = gitcmd.New("push", "--tags", "--force").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
			require.NoError(t, err)
...
func TestRepushTag(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, u *url.URL) {
		...
		_, _, err := gitcmd.New("tag", "v2.0").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
		assert.NoError(t, err)
		_, _, err = gitcmd.New("push", "origin", "--tags", "v2.0").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
		assert.NoError(t, err)
		...
		// delete the tag
		_, _, err = gitcmd.New("push", "origin", "--delete", "v2.0").RunStdString(t.Context(), &gitcmd.RunOpts{Dir: dstPath})
		assert.NoError(t, err)
