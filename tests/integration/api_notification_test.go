...
func TestAPINotification(t *testing.T) {
	...
	lastReadAt1 := "2000-01-01T00%3A50%3A01%2B00%3A00" // 946687801 <- only Notification 4 is in this filter ...
	req = NewRequest(t, "PUT", fmt.Sprintf("/api/v1/repos/%s/%s/notifications?last_read_at=%s&token=%s", user2.Name, repo1.Name, lastReadAt1, token))
	MakeRequest(t, req, http.StatusResetContent)

	lastReadAt2 := "946687801"
	req = NewRequest(t, "PUT", fmt.Sprintf("/api/v1/repos/%s/%s/notifications?last_read_at=%s&token=%s", user2.Name, repo1.Name, lastReadAt2, token))
	MakeRequest(t, req, http.StatusResetContent)
