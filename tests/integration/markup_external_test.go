	...
	t.Run("RenderContentInIFrame", func(t *testing.T) {
		t.Run("DefaultSandbox", func(t *testing.T) {
			...
			t.Run("SubPage", func(t *testing.T) {
				...
				assert.Equal(t, `<script type="module" src="/assets/`+public.GetAssetPath("js/external-render-iframe.js")+`"></script><link rel="stylesheet" href="/assets/`+public.GetAssetPath("css/external-render-iframe.css")+`"><div><any attr="val">&lt;script&gt;&lt;/script&gt;</any></div>`, respSub.Body.String())
			})
		})
		...
		t.Run("NoSanitizerNoSandbox", func(t *testing.T) {
			...
			t.Run("HTMLContentWithExternalRenderIframeHelper", func(t *testing.T) {
				...
				respSub := MakeRequest(t, req, http.StatusOK)
				assert.Equal(t, `<script type="module" src="/assets/`+public.GetAssetPath("js/external-render-iframe.js")+`"></script><link rel="stylesheet" href="/assets/`+public.GetAssetPath("css/external-render-iframe.css")+`"><script>foo("raw")</script>`, respSub.Body.String())
				assert.Equal(t, "frame-src 'self'", respSub.Header().Get("Content-Security-Policy"))
			})
		})
	})
