...
func TestAPIRestrictedUserLimitedOwner(t *testing.T) {
	...
	restrictedToken := getUserToken(t, "user29", auth_model.AccessTokenScopeReadUser, auth_model.AccessTokenScopeReadIssue)
	...
	viewerToken := getUserToken(t, "user4", auth_model.AccessTokenScopeReadUser, auth_model.AccessTokenScopeReadIssue)
	for _, path := range []string{
		"/api/v1/users/user2/activities/feeds",
		"/api/v1/users/user2/heatmap",
		"/api/v1/users/user2/keys",
