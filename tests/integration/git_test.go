...
func doCreateAgitFlowPull(dstPath string, ctx *APITestContext, baseBranch, headBranch string) func(t *testing.T) {
	return func(t *testing.T) {
		...
		t.Run("Push", func(t *testing.T) {
			err := git.NewCommand(git.DefaultContext, "push", "origin", "HEAD:refs/for/master", "-o").AddUntrustedArguments("topic=" + headBranch).Run(&git.RunOpts{Dir: dstPath})
			if !assert.NoError(t, err) {
				return
			}
			...
			_, _, err = git.NewCommand(git.DefaultContext, "push", "origin").AddUntrustedArguments("HEAD:refs/for/master/test/" + headBranch).RunStdString(&git.RunOpts{Dir: dstPath})
			if !assert.NoError(t, err) {
				return
			}
