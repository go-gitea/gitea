...
func TestAPIDeleteFile(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, u *url.URL) {
		...
		token2 := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeDeleteRepository)
		// Get user4's token
		session = loginUser(t, user4.Name)
		token4 := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeDeleteRepository)
